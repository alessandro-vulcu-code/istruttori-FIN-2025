# Metodologia

## 583 - Metodologia

**A quale età avviene lo sviluppo prevalente delle capacità coordinative?**

- [x] A. Prima dei 5 anni
- [ ] B. Dai 14 ai 18 anni
- [ ] C. Dopo i 18 anni
- [ ] D. Dai 5 ai 12 anni

## 584 - Metodologia

**Apprendiamo movimenti consapevolmente quando**

- [ ] A. Copiamo le soluzioni di un altro
- [ ] B. Intuiamo un movimento nuovo mentre siamo in azione
- [ ] C. L’istruttore guida un nostro movimento
- [x] D. Ci rappresentiamo un movimento e lo riproduciamo individuando come punti di riferimento elementi percettivi

## 585 - Metodologia

**Apprendiamo movimenti per condizionamento quando**

- [ ] A. Copiamo le soluzioni di un altro
- [ ] B. Intuiamo un movimento nuovo mentre siamo in azione
- [ ] C. L’istruttore guida un nostro movimento
- [x] D. Ripetiamo soluzioni che precedentemente avevano recato un beneficio

## 586 - Metodologia

**Apprendiamo movimenti per imitazione quando**

- [x] A. Copiamo le soluzioni di un altro
- [ ] B. Intuiamo un movimento nuovo mentre siamo in azione
- [ ] C. L’istruttore guida il nostro movimento
- [ ] D. Ci rappresentiamo un movimento e lo riproduciamo individuando come punti di riferimento elementi percettivi

## 587 - Metodologia

**Apprendiamo movimenti per prove ed errori quando**

- [ ] A. Copiamo le soluzioni di un altro
- [x] B. Intuiamo un movimento nuovo mentre siamo in azione
- [ ] C. L’istruttore guida un nostro movimento
- [ ] D. Ci rappresentiamo un movimento e lo riproduciamo individuando come punti di riferimento elementi percettivi

## 588 - Metodologia

**Caratteristiche essenziali delle quattro nuotate (crawl, dorso, farfalla, rana)**

- [ ] A. Ampiezza, continuità e ritmo.
- [x] B. Stabilità nella struttura e variabilità nei dettagli.
- [ ] C. Stabilità nei dettagli e variabilità nella struttura
- [ ] D. Non esistono caratteristiche comuni

## 589 - Metodologia

**Che cos’è la capacità di resistenza?**

- [ ] A. La capacità di resistere alla fatica nell’eseguire molte volte un medesimo gesto
- [x] B. La capacità di resistere alla fatica in esercitazioni di lunga durata
- [ ] C. La capacità di resistere alla fatica nell’eseguire molte volte un breve sforzo
- [ ] D. La capacita di resistere alla fatica mantenendo a lungo una buona coordinazione del gesto

## 590 - Metodologia

**Che cosa è la supercompensazione?**

- [ ] A. Una tabella valutativa per stabilire l’incremento delle capacità coordinative
- [ ] B. Una tecnica basata sulla respirazione per compensare il consumo di ossigeno dopo una seduta di allenamento
- [ ] C. Una capacità condizionale
- [x] D. Un principio dell’allenamento

## 591 - Metodologia

**Che cosa è più efficace per la correzione di un errore?**

- [ ] A. Ipercorrezione
- [x] B. Esercizi volti a migliorare la sensibilità dell’allievo
- [ ] C. Dimostrazioni corrette del movimento
- [ ] D. Spiegazioni tecniche dettagliate

## 592 - Metodologia

**Che cosa s’intende per analizzatore vestibolare?**

- [x] A. Una struttura che ci informa sulle accelerazioni del corpo
- [ ] B. Una struttura dalla quale riceviamo informazioni sulle tensione dei muscoli
- [ ] C. Un sinonimo di feedback
- [ ] D. Una struttura che fornisce l’informazione relativa alle pressioni esercitate sulle varie parti del corpo

## 593 - Metodologia

**Che errore non va commesso nell’allenamento giovanile?**

- [ ] A. Proporre i programmi degli atleti adulti in forma ridotta
- [ ] B. Preconizzare alcune fasi dello sviluppo motorio
- [ ] C. Automatizzare rigidamente il gesto tecnico
- [x] D. Tutte le risposte precedenti

## 594 - Metodologia

**Che tipo di scambi termici avvengono in acqua?**

- [x] A. Per conduzione e per convezione
- [ ] B. Per irraggiamento e conduzione
- [ ] C. Per convezione e per irraggiamento
- [ ] D. Per regolazione e rifinitura

## 595 - Metodologia

**Chi governa e gestisce gli analizzatori?**

- [x] A. Il cervello
- [ ] B. L’apparato osteo-articolare
- [ ] C. Il cuore
- [ ] D. L’apparato del Golgi

## 596 - Metodologia

**Classificazione della forza**

- [x] A. Forza massimale, forza rapida, resistenza alla forza
- [ ] B. Forza massimale, resistenza rapida, resistenza veloce
- [ ] C. La forza non può essere classificata
- [ ] D. Nel nuoto abbiamo una forza massima e una forza veloce

## 597 - Metodologia

**Com’è classificabile una virata nel nuoto?**

- [ ] A. Schema motorio di base
- [ ] B. Capacità coordinativa generale
- [ ] C. Capacità coordinativa speciale
- [x] D. Abilità motoria

## 598 - Metodologia

**Come può essere sviluppata la resistenza di breve durata?**

- [ ] A. Con la preacrobatica intensiva
- [ ] B. Con il metodo continuo
- [x] C. Con il metodo delle ripetizioni in serie
- [ ] D. Con diverse serie di tuffi di partenza

## 599 - Metodologia

**Come si può definire il carico fisico?**

- [x] A. L’insieme degli stimoli allenanti
- [ ] B. La durata delle esercitazioni
- [ ] C. Il rapporto tra esercitazioni e recupero
- [ ] D. La specificità delle esercitazioni

## 600 - Metodologia

**Come si può definire il nuoto?**

- [ ] A. Sport acquatico ad alta valenza educativa e coordinativa
- [ ] B. Sport educativo con spiccate capacità condizionali e coordinative
- [ ] C. Sport preventivo ai paramorfismi, di resistenza alla forza e alla velocità
- [x] D. Sport ciclico di resistenza alla forza ad elevata valenza coordinativa

## 601 - Metodologia

**Come si rendono coscienti le informazioni?**

- [ ] A. Con un lavoro molto ripetitivo
- [x] B. Con allenamenti mirati alla percezione
- [ ] C. Con un esecuzione perfetta
- [ ] D. Con una dialettica raffinata

## 602 - Metodologia

**Come viene classificata la forza?**

- [ ] A. Forza massimale, forza rapida, forza veloce, forza minima e resistenza alla forza
- [ ] B. Forza massimale, forza rapida, forza veloce e forza minima
- [x] C. Forza massimale, forza rapida o veloce e resistenza alla forza
- [ ] D. Forza massimale, forza minima, forza rapida e resistenza alla forza

## 603 - Metodologia

**Con quale attività si realizza lo sviluppo delle capacità coordinative?**

- [x] A. Multilaterale
- [ ] B. Multimediale
- [ ] C. Multidisciplinare
- [ ] D. Multitecnica

## 604 - Metodologia

**Correre è**

- [ ] A. Una predisposizione al movimento
- [x] B. Uno schema motorio di base
- [ ] C. Uno schema motorio evoluto
- [ ] D. Nessuna delle tre opzioni

## 605 - Metodologia

**Correre è**

- [ ] A. Una predisposizione al movimento
- [ ] B. Uno stato mentale
- [x] C. Uno schema motorio di base
- [ ] D. Uno schema motorio evoluto

## 606 - Metodologia

**Corretta procedura di correzione dell’errore**

- [x] A. Individuazione – destrutturazione del gesto – programma motorio e prima esecuzione – feedback e nuovo programma motorio – esecuzioni successive e nuovi feedback – output di migliore qualità
- [ ] B. Individuazione – programma motorio e prima esecuzione – destrutturazione del gesto – feedback e nuovo programma motorio – esecuzioni successive e nuovi feedback – output di migliore qualità
- [ ] C. Programma motorio e prima esecuzione – individuazione – destrutturazione del gesto – feedback e nuovo programma motorio – esecuzioni successive e nuovi feedback – output di migliore qualità
- [ ] D. Programma motorio e prima esecuzione – individuazione – feedback e nuovo programma motorio – esecuzioni successive e nuovi feedback – destrutturazione del gesto - output di migliore qualità

## 607 - Metodologia

**Cos’è il controllo, inteso come fattore della prestazione?**

- [ ] A. La capacità di controllare e monitorare il proprio corpo e l’ambiente circostante
- [x] B. L’insieme dei processi emotivi, cognitivi e motivazionali
- [ ] C. L’insieme dei processi emotivi e cognitivi
- [ ] D. La capacità di saper controllare la fatica durante ripetuti impegni di forza

## 608 - Metodologia

**Cos’è il tono muscolare?**

- [ ] A. La forza di un muscolo
- [ ] B. L’angolazione reciproca dei muscoli scheletrici
- [ ] C. La sezione trasversale di un muscolo
- [x] D. Il grado di tensione a riposo di un muscolo

## 609 - Metodologia

**Cos’è la resistenza aerobica?**

- [x] A. La capacità condizionale che garantisce di effettuare sforzi di lunga durata nella massima economia delle funzioni
- [ ] B. La capacità coordinativa grazie alla quale possiamo aumentare la frequenza dei nostri movimenti
- [ ] C. La capacità motoria che assicura di tollerare a lungo la fatica resistendo all’accumulo dell’acido lattico prodotto
- [ ] D. La capacità tipica del velocista

## 610 - Metodologia

**Cos’è la supercompensazione?**

- [ ] A. Un sistema gratificante per gli allievi
- [ ] B. Una limitazione all’adattamento dei carichi positivi
- [ ] C. Un principio che regola l’adattamento coordinativo
- [x] D. Un processo di adattamento dell’organismo ad uno stimolo allenante

## 611 - Metodologia

**Cos’è un movimento di transizione?**

- [x] A. Un movimento accessorio inserito dopo l’insegnamento di un gesto delle gambe
- [ ] B. Un movimento per predisporsi a quello successivo
- [ ] C. Un movimento concluso, inserito in una sequenza programmata
- [ ] D. Un movimento irrilevante ai fini della propulsione

## 612 - Metodologia

**Cos’è una fase sensibile?**

- [ ] A. Un periodo di malessere generalizzato
- [x] B. Un periodo particolarmente adatto per lo sviluppo di determinate capacità
- [ ] C. Un momento di stasi della crescita
- [ ] D. Un periodo poco favorevole per lo sviluppo di determinate capacità

## 613 - Metodologia

**Cosa s’intende per capacità di reazione?**

- [x] A. La capacità ad eseguire rapidamente azioni motorie adeguate in risposta ad un segnale
- [ ] B. La capacità di variare i movimenti del corpo nello spazio e nel tempo in risposta ad un segnale
- [ ] C. La capacità di regolare la propria concentrazione
- [ ] D. La capacità di coordinare tra loro i movimenti parziali del corpo reagendo ad un segnale

## 614 - Metodologia

**Cosa s’intende per densità?**

- [x] A. La quantità di materia presente in un volume
- [ ] B. la forza esercitata da un fluido in quiete su ogni superficie a contatto con esso
- [ ] C. La viscosità dell’acqua
- [ ] D. È quella forza che si oppone al movimento di un corpo in un fluido

## 615 - Metodologia

**Cosa s’intende per equilibrio?**

- [x] A. La funzione che consente di stare in una posizione
- [ ] B. I movimenti che consentono la stabilità
- [ ] C. La postura che si assume
- [ ] D. La stabilità emotiva

## 616 - Metodologia

**Cosa s’intende per età funzionale?**

- [x] A. L’espressione dell’interazione tra età biologica e quella psicologica
- [ ] B. La capacità di adattamento individuale
- [ ] C. La mera età dettata dal calendario
- [ ] D. L’età di acquisizione degli schemi motori di base

## 617 - Metodologia

**Cosa s’intende per forza rapida?**

- [ ] A. Capacità di resistere alla fatica con la maggior rapidità possibile
- [ ] B. Capacità di resistere alla rapidità con la maggior forza possibile
- [x] C. Capacità di produrre una forza da elevata a massima nel più breve tempo possibile
- [ ] D. Si sviluppa nei bambini di massimo 6 anni

## 618 - Metodologia

**Cosa s’intende per indipendenza segmentarla?**

- [ ] A. La capacità di eseguire movimenti inconsci
- [x] B. La capacità di percepire e controllare i segmenti corporei impegnati nel movimento in maniera indipendente
- [ ] C. La capacità dell’allievo di percepire il proprio movimento in maniera globale ed indifferenziata
- [ ] D. La capacità di frazionare l’esercizio nelle sue componenti fondamentali

## 619 - Metodologia

**Cosa s’intende per pressione idrostatica?**

- [ ] A. La quantità di materia presente in un volume
- [x] B. La forza esercitata da un fluido in quiete su ogni superficie a contatto con esso
- [ ] C. La viscosità dell’acqua
- [ ] D. È quella forza che si oppone al movimento di un corpo in un fluido

## 620 - Metodologia

**Cosa s’intende per resistenza in acqua?**

- [ ] A. La quantità di materia presente in un volume
- [ ] B. La forza esercitata da un fluido in quiete su ogni superficie a contatto con esso
- [ ] C. La viscosità dell’acqua
- [x] D. È quella forza che si oppone al movimento di un corpo in un fluido

## 621 - Metodologia

**Cosa si intende per resistenza alla forza?**

- [ ] A. La forza più elevata che il sistema neuromuscolare può esprimere
- [ ] B. La forza quanto più vicina possibile alla più elevata che il sistema neuromuscolare può esprimere
- [x] C. La capacità dell’organismo di opporsi alla fatica durante prestazioni di forza
- [ ] D. La capacità dell’organismo di opporsi alla resistenza durante prestazioni di forza rimanendo in regime aerobico

## 622 - Metodologia

**Cosa sono i fondamentali tecnici?**

- [ ] A. Tecniche di nuoto formatesi con numerose ripetizioni automatiche
- [ ] B. Fattori che determinano la capacità di apprendimento
- [x] C. Azioni motorie automatizzate mediante ripetizioni consapevoli
- [ ] D. Tecniche natatorie di atleti evoluti

## 623 - Metodologia

**Cosa sono le abilità motorie?**

- [x] A. Competenze relative a gesti motori specifici (azioni), si costruiscono sulle capacità coordinative e sono tante quante se ne possono apprendere
- [ ] B. Il presupposto per l’organizzazione e la regolazione del movimento: si dividono in generali e speciali e sono tante quante se ne possono apprendere
- [ ] C. Le competenze raggiunte in una specifica sequenza di movimenti (azioni), si costruiscono sulle capacità condizionali e sono tante quante se ne possono apprendere
- [ ] D. Movimenti mediamente finalizzati

## 624 - Metodologia

**Cosa sono le capacità motorie?**

- [ ] A. Abilità specifiche del singolo sport
- [ ] B. Abilità trasferibili a diverse discipline sportive
- [x] C. Presupposti motori fondamentali dell’essere umano
- [ ] D. Indici ottimali riferiti alla costituzione somatica dell’individuo

## 625 - Metodologia

**Da cosa è composto un analizzatore?**

- [ ] A. Dagli organi recettori
- [ ] B. Dalle strutture muscolo-tendinee
- [ ] C. Da due o più recettori e un sistema di trasmissione
- [x] D. Da due o più recettori, vie di comunicazione del segnale, centri del sistema nervoso deputati alla decodificazione di segnali

## 626 - Metodologia

**Definizione di resistenza aerobica**

- [x] A. La capacità di protrarre nel tempo la prestazione sportiva
- [ ] B. La capacità di ricostituire l’ATP
- [ ] C. La capacità di superare la soglia aerobica
- [ ] D. La capacità di raggiungere un alto livello di prestazione

## 627 - Metodologia

**Degli stimoli fisici troppo distanti tra loro comportano**

- [ ] A. Adattamenti momentanei e non permanenti
- [ ] B. Adattamenti condizionati
- [x] C. Nessun adattamento
- [ ] D. Ottimizzazione degli effetti della supercompensazione

## 628 - Metodologia

**Dopo il cambio di direzione nella virata a stile libero, come deve avvenire la spinta del corpo?**

- [ ] A. Già in posizione prona, ruotando prima della spinta dal bordo
- [ ] B. Già in posizione prona, ruotando durante la spinta dal bordo
- [ ] C. Con il piede dominante
- [x] D. Su un fianco, effettuando la rotazione durante lo scivolamento

## 629 - Metodologia

**Dove si inseriscono le correzioni e i nuovi apprendimenti?**

- [x] A. Nella zona di sforzo fisico bassissimo
- [ ] B. Nella zona di sforzo fisico altissimo
- [ ] C. A secco
- [ ] D. Nella zona di sforzo fisico medio alto

## 630 - Metodologia

**Durante la fase di apprendimento motorio della coordinazione grezza, quali aspetti degli elementi fondamentali del movimento bisogna tenere in considerazione?**

- [x] A. Ampiezza e continuità
- [ ] B. Forza e velocità
- [ ] C. Precisione e perfezione
- [ ] D. Motivazione e condizione

## 631 - Metodologia

**È giusto richiedere sin dall’inizio l’esecuzione ad alta velocità di nuotate grezze?**

- [ ] A. Sì, perché così viene arricchito il patrimonio genetico di fibre bianche
- [ ] B. Sì, sempre
- [ ] C. Sì, ma solo con allievi molto coordinati
- [x] D. No, perché non è ancora stata raggiunta la piena indipendenza segmentaria

## 632 - Metodologia

**Fra i modelli di allenamento che cosa indica la sigla C2?**

- [ ] A. Tolleranza al lattato
- [ ] B. Esercizi di velocità
- [x] C. Picco di lattato
- [ ] D. Intensità intorno alla soglia anaerobica

## 633 - Metodologia

**Gli adattamenti generati dall’allenamento**

- [ ] A. Dipendono dalle caratteristiche dell’allenamento svolto
- [ ] B. Sono transitori per quanto riguardano le capacità condizionali
- [ ] C. Rispettano eterocronismi differenti
- [x] D. Tutte le precedenti

## 634 - Metodologia

**Gli analizzatori si classificano in**

- [ ] A. Primari e secondari
- [x] B. Ottico, acustico, tattile, cinestesico, labirintico
- [ ] C. Ottico e acustico
- [ ] D. Volontari e involontari

## 635 - Metodologia

**Gli analizzatori sono**

- [x] A. Strutture deputate alla selezione di stimoli
- [ ] B. Schemi che analizzano i movimenti
- [ ] C. Strutture di informazioni per correggere gli errori
- [ ] D. Abilità acquatiche apprese in forma cosciente

## 636 - Metodologia

**Gli analizzatori**

- [x] A. Trasmettono informazioni al SNC
- [ ] B. Segnalano la stanchezza muscolare
- [ ] C. Sono presenti solo in età adulta
- [ ] D. Sono presenti solo durante l’infanzia

## 637 - Metodologia

**Gli effetti del carico sono**

- [x] A. Immediati, permanenti, cumulativi
- [ ] B. Immediati e cumulativi ma non permanenti
- [ ] C. Permanenti e immediati ma non cumulativi
- [ ] D. Immediati e permanenti ma non cumulativi

## 638 - Metodologia

**Gli errori possono essere generati da**

- [ ] A. Scarso livello di attenzione
- [ ] B. Inadeguata rappresentazione mentale del gesto
- [ ] C. Scarsa propriocezione
- [x] D. Tutte le risposte precedenti

## 639 - Metodologia

**Gli esercizi di coordinazione devono essere rapportati**

- [x] A. Al grado di istruzione tecnica dell’allievo
- [ ] B. Al grado di istruzione tecnica dell’istruttore
- [ ] C. Al livello di interesse manifestato dall’allievo
- [ ] D. Al livello tecnico del corso

## 640 - Metodologia

**Gli obiettivi fondamentali nella didattica della bracciata a crawl sono**

- [ ] A. Ampiezza e frequenza della bracciata
- [ ] B. Cura dei giusti angoli e traiettorie
- [ ] C. Recupero rilassato e giusta posizione del capo
- [x] D. Cura della continuità delle azioni, corretto ritmo ed ampiezza

## 641 - Metodologia

**Gli schemi motori acquatici sono**

- [ ] A. Schemi che subiscono variazioni in base al sesso e all’età degli allievi
- [x] B. Fondamentali ai fini dello star bene in acqua e tra questi ricordiamo la capacità di immergersi, rilassarsi, spostarsi, galleggiare ecc.
- [ ] C. Fondamentali ai fini dello star bene in acqua e tra questi ricordiamo la capacità di immergersi, saltare, strisciare, camminare, chiudere gli occhi ecc.
- [ ] D. Schemi non sempre determinanti per lo stare bene in acqua

## 642 - Metodologia

**I fattori della prestazione sono**

- [ ] A. Forza, resistenza, velocità
- [ ] B. Centrati sulla mobilità articolare e la forza
- [ ] C. Coordinazione, Condizione, Costituzione, Capacità
- [x] D. Coordinazione, Costituzione, Condizione, Controllo

## 643 - Metodologia

**I meccanismi preposti alla risintesi dell’ATP sono**

- [ ] A. Infiniti
- [ ] B. Relazionabili all’età dell’atleta
- [ ] C. Due
- [x] D. Tre

## 644 - Metodologia

**I parametri del carico sono**

- [ ] A. Ampiezza, continuità e ritmo
- [ ] B. Intensità, densità, durata, volume: anche se il volume è il parametro prioritario di importanza
- [x] C. Intensità, densità, durata, volume: con priorità uguali di importanza
- [ ] D. Nella scuola nuoto non ci si deve porre il problema

## 645 - Metodologia

**I preadolescenti, dal punto di vista del sistema cardio-circolatorio, sono caratterizzati da**

- [ ] A. Una scarsa capacità di adattamento
- [ ] B. Nessuna capacità di adattamento
- [x] C. Buone capacità di adattamento
- [ ] D. Buone capacità di adattamento ma solo nei maschi

## 646 - Metodologia

**Il beccheggio è**

- [ ] A. Il movimento attorno all’asse sagittale
- [x] B. Il movimento attorno all’asse trasversale
- [ ] C. Il movimento attorno all’asse longitudinale
- [ ] D. L’ondulazione del corpo

## 647 - Metodologia

**Il carico esterno è misurabile con**

- [x] A. Metri percorsi
- [ ] B. Frequenza cardiaca
- [ ] C. Intensità dello sforzo
- [ ] D. Intertempi di percorrenza

## 648 - Metodologia

**Il carico fisico è**

- [ ] A. L’insieme degli stimoli di allenamento che non provocano la risposta adattativa dell’organismo
- [x] B. L’insieme degli stimoli di allenamento
- [ ] C. stimoli di allenamento che non fanno riferimento all’intervallo di recupero
- [ ] D. L’insieme degli stimoli di allenamento che non fanno riferimento alla frequenza settimanale

## 649 - Metodologia

**Il galleggiamento del corpo avviene quando**

- [x] A. Il suo peso specifico è minore del peso specifico dell’acqua
- [ ] B. Il suo peso specifico è maggiore del peso specifico dell’acqua
- [ ] C. Si esercita una pressione verso il basso utilizzando movimenti di remata
- [ ] D. Vengono applicati sostegni al baricentro

## 650 - Metodologia

**Il massimo livello di coordinazione è**

- [ ] A. Quello che ci permette di nuotare lunghe distanze
- [ ] B. Quello stabilito dal regolamento tecnico
- [x] C. Quello che consente nuovi e più fini movimenti
- [ ] D. Quello che si riscontra nella fase della coordinazione grezza

## 651 - Metodologia

**Il meccanismo anaerobico alattacido**

- [x] A. Interessa i substrati ATP+CP
- [ ] B. Ha durata di 20 secondi
- [ ] C. Riguarda la minima potenza energetica
- [ ] D. Se interessa la scissione del glicogeno dura circa 20-30 minuti

## 652 - Metodologia

**Il meccanismo energetico aerobico interviene prevalentemente**

- [x] A. Nei lavori di lunga durata e intensità moderata
- [ ] B. Nei lavori di breve durata e intensità elevata
- [ ] C. Nei tuffi
- [ ] D. Nelle apnee

## 653 - Metodologia

**Il metabolismo aerobico**

- [ ] A. Utilizza la fosfocreatina
- [x] B. Necessita della presenza di ossigeno
- [ ] C. Produce acido lattico
- [ ] D. È tipico degli sport di brevissima e breve durata

## 654 - Metodologia

**Il movimento volontario in acqua**

- [x] A. Non è soggetto alla forza di gravità, è generato da agonisti e antagonisti, è frenato dalla resistenza in acqua
- [ ] B. È soggetto alla forza di gravità, è generato dai muscoli agonisti, non è frenato dalla resistenza in acqua
- [ ] C. Ha le stesse caratteristiche di quello sulla terraferma
- [ ] D. È soggetto alla forza di gravità, è controllato dal muscolo controgravitario, la contrazione è maggiore all’inizio del movimento

## 655 - Metodologia

**Il perfezionamento della tecnica si propone di**

- [ ] A. Rendere più armonico il gesto natatorio
- [x] B. Aumentare l’efficacia della propulsione
- [ ] C. Rendere lo stile più adeguato al regolamento federale
- [ ] D. Rendere la nuotata esteticamente gradevole

## 656 - Metodologia

**Il principio di Archimede dice che**

- [ ] A. Ad ogni azione corrisponde una reazione uguale e contraria
- [ ] B. E=mc 2
- [ ] C. Quando avviene un aumento nella pressione in un punto di un fluido confinato, tale aumento viene trasmesso anche ad ogni punto del contenitore
- [x] D. Un corpo immerso in un fluido riceve una spinta verticale, dal basso verso l’alto, pari al peso del fluido da esso spostato

## 657 - Metodologia

**Il progressivo incremento del carico, in generale, come deve avvenire?**

- [ ] A. Prima aumenta l’intensità, poi il volume
- [x] B. Prima aumenta il volume, poi l’intensità
- [ ] C. Prima aumenta la densità, poi il volume
- [ ] D. Intensità e volume aumentano contemporaneamente e in uguale misura

## 658 - Metodologia

**Il rendimento meccanico di un notatore evoluto è circa: #####**

- [ ] A. 1,0-1.2% #####
- [ ] B. 2,0-2,4% #####
- [x] C. 10,0-12,0% #####
- [ ] D. 20,0-24,0%

## 659 - Metodologia

**Il rollio è**

- [ ] A. L’ondulazione del corpo
- [ ] B. Il movimento attorno all’asse sagittale
- [ ] C. Il movimento attorno all’asse trasversale
- [x] D. Il movimento attorno all’asse longitudinale

## 660 - Metodologia

**Il rollio nello stile libero**

- [x] A. Si effettua sempre
- [ ] B. Si effettua solo nelle distanze lunghe
- [ ] C. Si effettua solo nelle distanze brevi
- [ ] D. Si effettua solo se la coordinazione è due colpi di gambe per un ciclo di bracciata

## 661 - Metodologia

**Il talento sportivo dipende da**

- [ ] A. Incremento delle capacità condizionali
- [ ] B. Nurturing neonatale
- [ ] C. Esclusivamente dall’affinamento della tecnica delle nuotate
- [x] D. Sviluppo di senso percezione, coordinazione e tecnica

## 662 - Metodologia

**In che modo è possibile definire il carico interno determinato da un’esercitazione?**

- [ ] A. Attraverso il numero delle ripetizioni del singolo esercizio
- [ ] B. Attraverso una RMF
- [ ] C. Attraverso il calcolo delle frequenze di bracciata
- [x] D. Attraverso il calcolo della frequenza cardiaca

## 663 - Metodologia

**In che relazione devono essere quantità ed intensità in un buon allenamento?**

- [ ] A. Elevata Intensità ed elevata quantità
- [x] B. Ridotta Intensità ed elevata quantità
- [ ] C. Ridotta Intensità e ridotta quantità
- [ ] D. Non hanno alcuna relazione

## 664 - Metodologia

**In cosa consiste lo schema corporeo?**

- [ ] A. La percezione del proprio corpo nello spazio e nel tempo
- [ ] B. Un’immagine di sé, vera rappresentazione topologica del corpo nello spazio
- [ ] C. In una rappresentazione anatomicamente dettagliata del corpo dell’allievo
- [x] D. Una rappresentazione cognitiva della posizione e dell’estensione del corpo nello spazio, dell’organizzazione gerarchica dei singoli segmenti corporei, in relazione ai dati provenienti dal mondo interno ed esterno, finalizzata all’organizzazione dell’azione nello spazio

## 665 - Metodologia

**In quale dei seguenti allenamenti si utilizza il sistema energetico anaerobico alattacido? #####**

- [x] A. C3 #####
- [ ] B. B1 #####
- [ ] C. A2 #####
- [ ] D. C2

## 666 - Metodologia

**In quale fascia d’età si sviluppa la forza rapida?**

- [ ] A. Tra i 5 e gli 8 anni
- [x] B. Tra gli 8 e i 12 anni
- [ ] C. Tra i 12 e i 16 anni
- [ ] D. Oltre i 16 anni

## 667 - Metodologia

**In quale fascia di età è consigliabile sviluppare le capacità coordinative?**

- [ ] A. Dai 16 anni in poi
- [x] B. Dai 6 agli 11 anni
- [ ] C. Dagli 0 ai 5 anni
- [ ] D. Non c’è un periodo particolare

## 668 - Metodologia

**In regime di C1 si lavora su**

- [ ] A. Intensità intorno alla soglia anaerobica
- [ ] B. Picco di lattato
- [x] C. Tolleranza al lattato
- [ ] D. Intensità intorno al massimo consumo di ossigeno

## 669 - Metodologia

**In una gara a farfalla è possibile nuotare sul fianco?**

- [ ] A. Sì
- [ ] B. No
- [ ] C. Solo nella vasca di ritorno
- [x] D. Solo nella fase subacquea

## 670 - Metodologia

**Individuare a quale concetto corrisponde la seguente definizione: sono le capacità di dosare, ottimizzare ed adattare il comportamento dell’uomo nello svolgere diversi compiti motori, mediante l’opportuna distribuzione temporale e spaziale degli impegni muscolari, in accordo alle sue capacità condizionali**

- [ ] A. Le capacità cinestesiche
- [ ] B. Gli analizzatori
- [ ] C. Le fasi sensibili
- [x] D. Le capacità coordinative

## 671 - Metodologia

**Inspirazione ed espirazione hanno gli stessi tempi esecutivi in acqua?**

- [ ] A. Sì, sempre
- [ ] B. Dipende dagli stili
- [x] C. No, l’inspirazione ha un tempo minore
- [ ] D. No, l’espirazione ha un tempo minore

## 672 - Metodologia

**L’allenamento giovanile, in particolare per gli Esordienti B, è finalizzato**

- [ ] A. Al raggiungimento della performance
- [x] B. All’incremento delle capacità coordinative
- [ ] C. Al perfezionamento della tecnica
- [ ] D. Alla convocazione nelle rappresentative regionali

## 673 - Metodologia

**L’allenamento specifico della forza può iniziare**

- [ ] A. Mai
- [ ] B. A 8 – 10 anni
- [ ] C. A 11 – 12 anni
- [x] D. A 13 – 15 anni

## 674 - Metodologia

**L’apprendimento della tecnica è influenzato da fattori**

- [x] A. Coordinativi e condizionali
- [ ] B. Interni ed esterni
- [ ] C. Muscolari ed organici
- [ ] D. Alimentari e psicologici

## 675 - Metodologia

**L’apprendimento motorio passa attraverso tre fasi**

- [ ] A. Fase della coordinazione grezza, coordinazione globale e coordinazione disponibile
- [x] B. Fase della coordinazione grezza, coordinazione fine e disponibilità variabile
- [ ] C. Fase della coordinazione grezza, coordinazione globale e coordinazione analitica
- [ ] D. Fase della costituzione – condizione – coordinazione - controllo

## 676 - Metodologia

**L’attivazione di un sistema energetico piuttosto che un altro dipende**

- [x] A. Dal livello di allenamento
- [ ] B. Dalla produzione di adrenalina
- [ ] C. Dal momento della giornata
- [ ] D. Dalle caratteristiche dell’esercizio fisico e dallo stato nutrizionale

## 677 - Metodologia

**L’attività sportiva per l’adolescente può**

- [ ] A. Essere considerata uno svago
- [ ] B. Avere influenza sul piano fisico ma non su quello cognitivo
- [x] C. Migliorare l’immagine corporea
- [ ] D. (Solo negli sport di squadra) Sviluppare l’ambito socio-affettivo e relazionale

## 678 - Metodologia

**L’equilibrio in acqua si ottiene**

- [ ] A. Mantenendo una posizione consona e reagendo allo squilibrio con una contrazione contro laterale
- [ ] B. Con la frequenza di un corso intensivo
- [x] C. In posizione verticale in rilassamento generale in altre posizioni opportune mantenendo una posizione consona
- [ ] D. Solo in galleggiamento prono e supino

## 679 - Metodologia

**L’equilibrio**

- [ ] A. È la capacità di eseguire un compito motorio mantenendo il corpo in equilibrio statico
- [ ] B. È la capacità di eseguire un compito motorio mantenendo il corpo in equilibrio dinamico
- [ ] C. È regolato prevalentemente dalle informazioni dell’analizzatore cinestesico e tattile
- [x] D. È regolato prevalentemente dalle informazioni dell’analizzatore ottico e vestibolare

## 680 - Metodologia

**L’errore sportivo è**

- [ ] A. L’esecuzione involontariamente diversa dalla richiesta tecnica avuta
- [ ] B. Il risultato di una distrazione
- [x] C. L’allontanamento dal modello prestativo
- [ ] D. L’interpretazione non corretta della richiesta tecnica

## 681 - Metodologia

**L’espressione motoria tipica dei bambini e dei preadolescenti è**

- [x] A. La rapidità
- [ ] B. La forza
- [ ] C. La tenacia
- [ ] D. La resistenza

## 682 - Metodologia

**L’insieme delle strutture cerebrali deputate al controllo del movimento si chiama**

- [ ] A. Sistema di raccolta, progettazione e regolazione del movimento
- [ ] B. Sistema sensibile
- [ ] C. Sistema olistico
- [x] D. Sistema sensomotorio

## 683 - Metodologia

**L’interruzione della battuta di gambe a crawl è solitamente imputabile a**

- [ ] A. Limitata rotazione spalle durante la respirazione che provoca la rotazione dell’anca con conseguente incrocio delle gambe
- [ ] B. Pigrizia mentale del nuotatore
- [x] C. Mancanza di coordinazione braccia/gambe
- [ ] D. Minore propulsione della bracciata

## 684 - Metodologia

**L’ontogenesi della motricità si compone di due fasi, quali?**

- [x] A. Maturazione anatomica – maturazione funzionale
- [ ] B. Maturazione organica – maturazione psichica
- [ ] C. Maturazione biologica – maturazione chimico-fisica
- [ ] D. Maturazione endogena – maturazione esogena

## 685 - Metodologia

**L’organizzazione delle sensazioni relative al proprio corpo in rapporto con i dati del mondo esterno definisce**

- [x] A. Lo schema corporeo
- [ ] B. Le capacità coordinative
- [ ] C. Lo schema motorio
- [ ] D. L’età biologica

## 686 - Metodologia

**La coordinazione è la capacità di**

- [ ] A. Mantenere o riacquistare con il corpo una posizione stabile in differenti situazioni statiche o dinamiche
- [ ] B. Compiere atti motori utilizzando la massima escursione articolare
- [x] C. Regolare e dosare il complesso degli impegni muscolari onde ottenere l’azione con rapidità, precisione e armonia
- [ ] D. Ottimizzare il rapporto peso/potenza

## 687 - Metodologia

**La differenziazione cinestesica è**

- [ ] A. Un particolare analizzatore
- [ ] B. Una capacità condizionale non sempre allenabile
- [x] C. Una capacità coordinativa speciale
- [ ] D. Una capacità coordinativa generale

## 688 - Metodologia

**La diminuzione della frequenza cardiaca a riposo è sinonimo di**

- [ ] A. Uno stato di affaticamento
- [ ] B. Un incombente collasso cardiocircolatorio
- [x] C. Una migliore efficienza funzionale
- [ ] D. Una aumentata diffusione periferica del plasma

## 689 - Metodologia

**La fase della coordinazione fine**

- [x] A. Consente un movimento più fluido, armonico e compiuto
- [ ] B. Non conosce momenti di stasi
- [ ] C. Non è influenzata da fattori di disturbo
- [ ] D. È l’ultimo stadio del processo di apprendimento

## 690 - Metodologia

**La fase della coordinazione grezza**

- [x] A. È caratterizzata dall’attività di comprensione del compito motorio
- [ ] B. Non è subordinata alla dimostrazione pratica
- [ ] C. Migliora se non si effettuano ripetizioni riuscite e corrette
- [ ] D. È secondo una capacità coordinativa speciale

## 691 - Metodologia

**La fase della disponibilità variabile è**

- [ ] A. La fase in cui la rappresentazione del movimento diventa più completa
- [x] B. L’ultimo stadio del processo di apprendimento
- [ ] C. Lo stadio intermedio in cui è importante variare la disponibilità delle proprie abilità
- [ ] D. La fase in cui viene, erroneamente, automatizzato l’errore

## 692 - Metodologia

**La formazione di abilità motorie contribuisce a sviluppare le capacità coordinative?**

- [ ] A. Solo nell’apprendimento delle tecniche
- [ ] B. Sì, in base alle fasi sensibili
- [x] C. Sempre, in ogni caso
- [ ] D. No, le capacità coordinative si sviluppano in modo autonomo

## 693 - Metodologia

**La forza è**

- [x] A. Una capacità condizionale
- [ ] B. Una capacità coordinativa
- [ ] C. Una caratteristica genetica
- [ ] D. Una capacità mista

## 694 - Metodologia

**La forza può essere**

- [ ] A. Coordinata, psicologica, breve
- [ ] B. Intensa, lieve, media
- [ ] C. Condizionale, motoria, tenue
- [x] D. Massimale, resistente, veloce

## 695 - Metodologia

**La frequenza cardiaca**

- [x] A. Va controllata perché aiuta a valutare il diverso grado di affaticamento degli allievi
- [ ] B. Deve rimanere ai livelli elevati durante tutta la lezione
- [ ] C. Deve rimanere a livelli simili a quelli che di hanno durante il riposo
- [ ] D. È uguale per tutti i soggetti che effettuano gli stessi esercizi

## 696 - Metodologia

**La lentezza della nuotata a rana è prevalentemente dovuta a**

- [ ] A. Extrarotazione dei piedi durante il colpo di gambe
- [ ] B. Mancanza della fase di spinta degli arti inferiori
- [ ] C. Recupero subacqueo degli arti inferiori
- [x] D. Alternanza delle azioni propulsive su un assetto di galleggiamento non ideale

## 697 - Metodologia

**La mobilità articolare è**

- [ ] A. L’esecuzione veloce di movimenti ampi
- [x] B. La capacità di eseguire movimenti con la massima ampiezza
- [ ] C. La capacità di eseguire movimenti con la massima velocità
- [ ] D. Una circostanza che aumenta l’economia del gesto e facilita l’affaticamento

## 698 - Metodologia

**La mobilità articolare è**

- [ ] A. La capacità di compiere azioni rapidamente
- [x] B. La capacità di compiere gesti con la massima escursione
- [ ] C. La capacità di muovere gli arti singolarmente
- [ ] D. La capacità d vincere una resistenza in un tempo breve

## 699 - Metodologia

**La motivazione**

- [x] A. È alla base dell’apprendimento
- [ ] B. È legata all’età dell’individuo
- [ ] C. Non conosce fattori inibenti
- [ ] D. Può essere trascurata relativamente all’apprendimento tecnico

## 700 - Metodologia

**La motricità si basa**

- [x] A. Sulle funzioni del SNC e degli organi di senso
- [ ] B. Sull’attività della muscolatura
- [ ] C. Sull’intelligenza speculativa
- [ ] D. Sulle funzioni organiche e muscolari

## 701 - Metodologia

**La multilateralità può essere di tipo**

- [ ] A. Simbolico
- [ ] B. Motorio e propedeutico
- [ ] C. Proteico
- [x] D. Generale e mirato

## 702 - Metodologia

**La multilateralità può essere salvaguardata, durante l’allenamento**

- [ ] A. Attraverso esercizi che prevedano il nuoto in tutti gli stili
- [x] B. Attraverso la sintesi di due principi : ricchezza di mezzi e alternanza di esercizi
- [ ] C. Facendo nuotare ai giovani tutte le distanze a stile libero
- [ ] D. Attraverso l’utilizzo di serie di allenamento molto frazionate

## 703 - Metodologia

**La percezione è**

- [x] A. La funzione che ci permette di raccogliere informazioni relative a ciò che succede dentro il nostro corpo e a quello che succede all’esterno
- [ ] B. La stimolazione sensoriale che non raggiunge i livelli della coscienza
- [ ] C. La funzione che indica il livello di tensione di un muscolo scheletrico
- [ ] D. Una capacità coordinativa speciale

## 704 - Metodologia

**La prestazione motoria dipende da**

- [ ] A. Costituzione, condizione, coordinazione e valutazione
- [ ] B. Costituzione, condizione, sperequazione e controllo
- [ ] C. Costituzione, valutazione, sperequazione e coordinazione
- [x] D. Costituzione, condizione, coordinazione e controllo

## 705 - Metodologia

**La programmazione consente di**

- [x] A. Definire procedure e mezzi per favorire l’apprendimento
- [ ] B. Definire la crescita della personalità dell’allievo
- [ ] C. Abbreviare la progressione didattica
- [ ] D. Programmare un ruolo più attivo dell’allievo più dotato

## 706 - Metodologia

**La resistenza d’onda è dovuta**

- [ ] A. Alla particolare resistenza della superficie dell’acqua.
- [ ] B. Ai movimenti circolari dell’acqua che risucchiano il corpo.
- [x] C. Ai movimenti superficiali dell’acqua che condizionano l’avanzata. del corpo
- [ ] D. Alla percentuale di massa grassa del nuotatore

## 707 - Metodologia

**La resistenza di attrito è dovuta**

- [ ] A. Alla superficie del piano perpendicolare all’asse di avanzamento
- [x] B. Allo sfregamento del corpo con le molecole d’acqua
- [ ] C. Ai movimenti superficiali dell’acqua che condizionano l’avanzata. del corpo
- [ ] D. Alla percentuale di massa grassa del nuotatore

## 708 - Metodologia

**La resistenza di superficie o tensione superficiale è dovuta**

- [x] A. Alla particolare resistenza della superficie dell’acqua
- [ ] B. Ai movimenti circolari dell’acqua che risucchiano il corpo
- [ ] C. Allo sfregamento del corpo con le molecole d’acqua
- [ ] D. Alla percentuale di massa grassa del nuotatore

## 709 - Metodologia

**La resistenza di vortice, o turbolenza, è dovuta**

- [ ] A. Alla superficie del piano perpendicolare all’asse di avanzamento
- [x] B. Ai movimenti circolari dell’acqua che risucchiano il corpo
- [ ] C. Ai movimenti superficiali dell’acqua che condizionano l’avanzata del corpo
- [ ] D. Alla percentuale di massa grassa del nuotatore

## 710 - Metodologia

**La resistenza è**

- [ ] A. Una capacità coordinativa
- [x] B. Una capacità condizionale
- [ ] C. Una condizione psicomotoria
- [ ] D. Una tecnica psicologica

## 711 - Metodologia

**La resistenza frontale all’avanzamento in acqua è dovuta**

- [x] A. Alla superficie del piano perpendicolare all’asse di avanzamento
- [ ] B. Alla particolare resistenza della superficie dell’acqua
- [ ] C. Ai movimenti circolari dell’acqua che risucchiano il corpo
- [ ] D. Alla tipologia di prodotti usati per la clorazione

## 712 - Metodologia

**La resistenza frontale, raddoppiando la velocità di avanzamento, aumenta di**

- [ ] A. 2 volte
- [ ] B. 3 volte
- [x] C. 4 volte
- [ ] D. 8volte

## 713 - Metodologia

**La sensazione è**

- [ ] A. La è la funzione che ci permette di raccogliere informazioni relative a ciò che succede dentro il nostro corpo e a quello che succede all’esterno
- [ ] B. Una componente della prestazione
- [x] C. La stimolazione sensoriale che non raggiunge i livelli della coscienza
- [ ] D. La premonizione di eventi futuri

## 714 - Metodologia

**La sensopercezione è**

- [x] A. Un’attività conoscitiva che permette di percepire gli stimoli provenienti dagli analizzatori
- [ ] B. Un esercizio per migliorare la sensibilità degli allievi
- [ ] C. Una capacità condizionale
- [ ] D. Un insieme di stimoli atti a migliorare la prestazione

## 715 - Metodologia

**La spinta di galleggiamento è**

- [ ] A. La forza applicata al baricentro
- [x] B. La forza applicata al centro di galleggiamento
- [ ] C. La combinazione tra forza di gravità e peso corporeo
- [ ] D. La forza espressa dalla gambata

## 716 - Metodologia

**La supercompensazione è**

- [ ] A. Un processo mediante il quale il disagio provoca una situazione di fatica , cui segue una reazione ricostruttiva che supera la fatica precedente
- [ ] B. Un processo in relazione al quale tutte le fatiche vengono superate attraverso stimolazioni muscolari
- [ ] C. Un processo di ricostruzione dello stimolo a seguito di carichi di lavoro di tipo C2
- [x] D. Il processo mediante il quale lo stimolo provoca una condizione di disagio (la fatica), cui segue una reazione ricostruttiva che supera il livello precedente alla stimolazione

## 717 - Metodologia

**La tecnica della nuotata a dorso prevede che**

- [ ] A. Le braccia siano perfettamente opposte
- [x] B. Un braccio entri in acqua prima dell’inizio della parte finale di spinta dell’altro
- [ ] C. La velocità condizioni le fasi della bracciata
- [ ] D. Siano deliberamente escluse esercitazioni sulla respirazione (il capo è emerso)

## 718 - Metodologia

**La tecnica di partenza denominata track start permette**

- [ ] A. Di aggrappare entrambi i piedi al blocco
- [ ] B. Un movimento più spettacolare
- [x] C. Di avere più equilibrio sul blocco
- [ ] D. Di applicare la spinta per un tempo più lungo

## 719 - Metodologia

**La tecnica natatoria ottimale è**

- [x] A. Stabile nella struttura e variabile nei dettagli
- [ ] B. Stabile nei dettagli e variabile nella struttura
- [ ] C. Stabile nella struttura e nei dettagli
- [ ] D. Variabile nella struttura e nei dettagli

## 720 - Metodologia

**La track start è**

- [ ] A. Un movimento che nella pallanuoto serve a smarcarsi
- [ ] B. Il passaggio di posizione da gambe allungate a gambe fuori nel nuoto sincronizzato
- [x] C. La partenza dal blocco dove i piedi non sono allineati
- [ ] D. Una presa per il trasporto del manichino nel salvamento sportivo

## 721 - Metodologia

**La variazione di un esercizio in una progressione didattica stimola maggiormente le capacità**

- [ ] A. Vestibolari
- [x] B. Coordinative
- [ ] C. Visive
- [ ] D. Condizionali

## 722 - Metodologia

**La velocità di una bracciata a crawl è**

- [ ] A. Non misurabile
- [ ] B. La stessa in ogni fase
- [x] C. Progressivamente accelerata
- [ ] D. Progressivamente decelerata

## 723 - Metodologia

**La velocità è**

- [x] A. La capacità di eseguire un gesto nel minor tempo possibile
- [ ] B. La capacità che ci permette di essere rapidi
- [ ] C. La capacità che ci permette di coordinare i movimenti
- [ ] D. Una capacità coordinativi

## 724 - Metodologia

**Le capacità condizionali dipendono principalmente**

- [ ] A. Da aspetti di tipo fisiologico, energetico, biochimico
- [ ] B. Da aspetti neuronali dettati dal sistema nervoso centrale
- [ ] C. Da aspetti socio-educativi
- [x] D. Da aspetti energetici, biologici, fisiologici e coordinativi

## 725 - Metodologia

**Le capacità condizionali si sviluppano**

- [ ] A. Da appena nati
- [x] B. Dai 10 anni
- [ ] C. Dai 4 ai 10 anni
- [ ] D. Esclusivamente in età adulta

## 726 - Metodologia

**Le capacità condizionali sono capacità fisiche determinate da**

- [x] A. Fattori energetici
- [ ] B. Fattori psicologici
- [ ] C. Fattori antropologici
- [ ] D. Fattori neurologici

## 727 - Metodologia

**Le capacità coordinative devono essere esercitate a partire da**

- [ ] A. 8 anni
- [x] B. 5/6 anni
- [ ] C. 12 anni
- [ ] D. 10 anni

## 728 - Metodologia

**Le capacità coordinative dipendono**

- [ ] A. Dalla sensibilità muscolare
- [x] B. Dalla funzionalità del sistema nervoso
- [ ] C. Dall’apparato cardiocorcolatorio
- [ ] D. Dall’età biologica

## 729 - Metodologia

**Le capacità coordinative generali riguardano**

- [ ] A. Forza, resistenza, velocità
- [ ] B. Orientamento, ritmo, mobilità articolare
- [ ] C. Destrezza, equilibrio, elasticità dei movimenti
- [x] D. Apprendimento, controllo, trasformazione dei movimenti

## 730 - Metodologia

**Le capacità coordinative**

- [x] A. Controllano, regolano ed organizzano il movimento
- [ ] B. Sono legate ad aspetti antropometrici ed energetici
- [ ] C. Sono legate al sistema nervoso centrale ma non periferico
- [ ] D. Controllano primariamente il movimento di arti inferiori e superiori

## 731 - Metodologia

**Le capacità motorie sono classificate in**

- [ ] A. Coordinative, compensative, condizionali
- [ ] B. Coordinative e condizionali
- [x] C. Coordinative, condizionali ed intermedie
- [ ] D. Capacità di compiere o inibire movimenti

## 732 - Metodologia

**Le capacità motorie sono**

- [ ] A. Apprendimento motorio
- [x] B. Mobilità articolare, coordinazione, condizione
- [ ] C. Forza, resistenza, velocità
- [ ] D. Costituzione, condizione, coordinazione, controllo

## 733 - Metodologia

**Le capacità sensopercettive dipendono da**

- [ ] A. L’istruttore
- [ ] B. L’ambiente
- [x] C. L’individuo
- [ ] D. Le capacità coordinative

## 734 - Metodologia

**Le capacità sensopercettive sono la base per la costruzione della**

- [ ] A. Abilità
- [x] B. Tecnica
- [ ] C. Tattica
- [ ] D. Affettività

## 735 - Metodologia

**Le esercitazioni sulla multilateralità incrementano**

- [ ] A. Lo sviluppo condizionale
- [x] B. Lo sviluppo coordinativo
- [ ] C. I fattori allenanti
- [ ] D. La mobilità articolare dei distretti coinvolti

## 736 - Metodologia

**Le fasi dello sviluppo della coordinazione sono**

- [ ] A. Controllo, adattamento e trasformazione
- [x] B. Coordinazione grezza, coordinazione fine e disponibilità variabile
- [ ] C. Iniziale, intermedia e finale
- [ ] D. Intensità, densità, volume

## 737 - Metodologia

**Le fasi sensibili**

- [ ] A. Sono legate all’attività degli analizzatori
- [ ] B. Servono per migliorare la sensibilità all’acqua
- [ ] C. Sono solo ipotizzate
- [x] D. Sono periodi dello sviluppo in cui alcuni stimoli provocano miglioramenti significativi

## 738 - Metodologia

**Le resistenze all’avanzamento, nel nuoto, sono in numero di: #####**

- [ ] A. 2 #####
- [x] B. 3 #####
- [ ] C. 4 #####
- [ ] D. 5

## 739 - Metodologia

**Le strutture incaricate alla selezione di stimoli si chiamano**

- [ ] A. Selezionatori
- [x] B. Analizzatori
- [ ] C. Elaboratori
- [ ] D. Commutatori

## 740 - Metodologia

**Lo schema corporeo è**

- [ ] A. Lo schema del sistema osteo- articolare
- [ ] B. Lo schema logico per organizzare la proposta di esercizi in acqua o in palestra
- [x] C. L’immagine che ognuno ha del proprio corpo in relazione con il mondo esterno
- [ ] D. L’immagine esterna del nostro corpo che non può essere percepita

## 741 - Metodologia

**Lo sviluppo delle capacità coordinative è importante nel nuoto**

- [x] A. Perché aiuta ad elaborare meglio la sensopercezione acquatica
- [ ] B. Perché incrementa le capacità di adattamento all’acqua
- [ ] C. Perché favorisce l’incremento della resistenza
- [ ] D. Perché favorisce l’accrescimento staturo-ponderale (legge di Stratz)

## 742 - Metodologia

**Nel crawl dopo il recupero aereo la mano che entra in acqua rispetto alla linea mediana del corpo è**

- [x] A. Inclinata con il palmo rivolto verso l’esterno
- [ ] B. Piatta e scivola verso avanti con il palmo rivolto verso il basso
- [ ] C. Inclinata con il palmo rivolto verso l’interno
- [ ] D. Tesa e piatta

## 743 - Metodologia

**Nel crawl la fase finale di spinta subacquea degli arti superiori avviene**

- [ ] A. Verso il basso per maggiore stabilità
- [ ] B. In modo costante per senso ritmico
- [x] C. In accelerazione
- [ ] D. Dal basso verso l’alto

## 744 - Metodologia

**Nel dorso, il rollio si esegue per ottenere**

- [ ] A. Rispetto dei regolamenti tecnici
- [x] B. Bracciata più potente in quanto i muscoli lavorano in condizioni biomeccaniche più vantaggiose
- [ ] C. Coordinazione 2/6
- [ ] D. Facilitazione nel posizionamento della testa, in questo modo si riesce a tenerla più bassa appoggiata all’acqua

## 745 - Metodologia

**Nel nuoto la tecnica ha come obiettivo**

- [x] A. Efficacia ed economicità del gesto
- [ ] B. Rapidità del gesto
- [ ] C. Massima forza massimale applicabile
- [ ] D. La distribuzione razionale dei carichi

## 746 - Metodologia

**Nel nuoto quali sono le articolazioni più stimolate?**

- [x] A. Polso, gomito, ginocchio e tibio- tarsica
- [ ] B. Falangi, gomito, coxo-femorale
- [ ] C. SNC
- [ ] D. Scapolo-omerale, colonna vertebrale, coxo-femorale e tibio-tarsica

## 747 - Metodologia

**Nel rapporto tra capacità e abilità motorie**

- [ ] A. L’apprendimento delle abilità motorie precede quello delle capacità coordinative
- [ ] B. Le abilità motorie si basano esclusivamente sulle capacità condizionali
- [x] C. Le abilità motorie si costruiscono sulle capacità motorie ma il loro esercizio aiuta lo sviluppo di queste ultime
- [ ] D. Non esistono rapporti strutturali tra capacità ed abilità motorie

## 748 - Metodologia

**Nell’allenamento giovanile quale tipo di forza è corretto sviluppare?**

- [ ] A. Forza massimale
- [x] B. Forza rapida e veloce
- [ ] C. Resistenza alla forza
- [ ] D. Forza multilaterale

## 749 - Metodologia

**Nell’esercizio di lunga durata**

- [x] A. L’energia è fornita dal meccanismo aerobico
- [ ] B. Si accumula acido lattico che impedisce di proseguire il lavoro
- [ ] C. Si utilizzano acidi grassi e proteine
- [ ] D. Si utilizza prevalentemente la fosfocreatina disponibile a livello muscolare

## 750 - Metodologia

**Nella battuta di gambe prona e supina, la propulsione è**

- [ ] A. Nulla, la gambata ha solo funzione di equilibrio
- [ ] B. Fornita dalla pianta del piede che schiaccia l’acqua
- [ ] C. Massimale
- [x] D. Fornita dal dorso del piede

## 751 - Metodologia

**Nella correzione di un errore nella bracciata a stile libero, bisogna concedere priorità a**

- [x] A. Continuità - ritmo
- [ ] B. Angoli - traiettorie
- [ ] C. Presa - appoggio
- [ ] D. Fase aerea (recupero)

## 752 - Metodologia

**Nella farfalla la gambata propulsiva avviene**

- [ ] A. All’inizio della fase inspiratoria
- [x] B. All’entrata in acqua delle mani
- [ ] C. Nella fase finale della trazione
- [ ] D. Durante la fase di trazione

## 753 - Metodologia

**Nella farfalla le mani appena entrano in acqua**

- [ ] A. Spingono verso il basso (downsweep)
- [x] B. Tirano verso fuori (outsweep)
- [ ] C. Tirano fino alle spalle
- [ ] D. Affondano repentinamente

## 754 - Metodologia

**Nella fase di coordinazione grezza cosa è bene correggere?**

- [ ] A. I dettagli
- [ ] B. Gli errori grossolani e i dettagli
- [ ] C. Tutto il possibile
- [x] D. Gli errori grossolani

## 755 - Metodologia

**Nella fase di spinta nel dorso, il palmo della mano è rivolto**

- [x] A. Verso i piedi
- [ ] B. Verso il fianco
- [ ] C. Verso l’alto
- [ ] D. Verso il basso

## 756 - Metodologia

**Nella fase subacquea a rana dopo la virata la corretta sequenza dei movimenti è**

- [ ] A. Presa, trazione, spinta, recupero
- [ ] B. Spinta dal bordo, azione di braccia gambata rana recupero delle braccia
- [ ] C. Spinta dal bordo, gambata a delfino, azione delle braccia, recupero, gambata rana
- [x] D. Spinta dal bordo, bracciata, colpo di gambe a delfino recupero delle braccia, gambata rana

## 757 - Metodologia

**Nella nuotata a crawl cosa è importante?**

- [ ] A. Consentire alla mano, durante il recupero, di sfiorare l’acqua per non perdere il contatto con la stessa
- [x] B. Effettuare l’ultima parte della passata subacquea con un azione accelerata
- [ ] C. Consentire al braccio di guidare il recupero
- [ ] D. Consentire alla mano, durante il recupero, di sfiorare l’acqua per prepararsi alla fase di presa

## 758 - Metodologia

**Nella nuotata a crawl quando deve iniziare la rotazione del capo?**

- [ ] A. Prima che l’arto opposto sia entrato in acqua
- [ ] B. Quando l’arto opposto inizia la fase di spinta
- [ ] C. Quando l’arto opposto è in fase di recupero
- [x] D. Dopo che l’arto opposto è entrato in acqua

## 759 - Metodologia

**Nella nuotata a dorso l’esercizio definito opposizione di fase è un**

- [ ] A. Esercizio di esclusione
- [x] B. Esercizio di perfezionamento
- [ ] C. Esercizio di specializzazione
- [ ] D. Esercizio inutile

## 760 - Metodologia

**Nella nuotata a rana il recupero degli arti inferiori avviene**

- [ ] A. Quando le braccia sono in fase di trazione
- [x] B. Quando le braccia sono nella fase del recupero
- [ ] C. Quando le braccia sono in fase di spinta
- [ ] D. Quando le braccia sono in fase di presa/appoggio

## 761 - Metodologia

**Nella nuotata a rana si può considerare errore**

- [ ] A. Inserire una pausa dopo il recupero di ogni bracciata
- [x] B. Sovrapporre l’azione propulsiva delle braccia a quella delle gambe
- [ ] C. Effettuare uno scivolamento dopo ogni gambata
- [ ] D. Respirare frontalmente

## 762 - Metodologia

**Nella nuotata a stile libero, un concorrente può rimanere in completa immersione**

- [ ] A. Per tutta la distanza che deve coprire
- [ ] B. Mai
- [ ] C. Per i primi 12,5 metri (mezza vasca)
- [x] D. Alla partenza ed alle virate, per un massimo di 15 metri

## 763 - Metodologia

**Nella preparazione infantile e giovanile, la formazione condizionale deve mirare allo sviluppo di**

- [ ] A. Forza veloce, resistenza anaerobica e resistenza alla forza
- [x] B. Forza rapida, resistenza aerobica e resistenza alla forza
- [ ] C. Forza rapida, resistenza anaerobica e resistenza alla forza
- [ ] D. Forza rapida, resistenza aerobica e resistenza alla fatica

## 764 - Metodologia

**Nella rana effettuare un forte sollevamento del busto consente al nuotatore**

- [x] A. Il recupero degli avambracci fuori dall’acqua
- [ ] B. Il recupero delle braccia fuori dall’acqua
- [ ] C. Di facilitare la fase di inspirazione durante il ciclo di bracciata
- [ ] D. Di controllare meglio gli avversari

## 765 - Metodologia

**Nella realizzazione del progetto d’azione, all’interno della programmazione dello schema motorio, possiamo affermare che**

- [ ] A. La riformulazione del progetto d’azione si verifica spontaneamente
- [ ] B. Una volta formato lo schema motorio esso è immodificabile per tutta la vita del soggetto.
- [x] C. È possibile una riformulazione del progetto d’azione e quindi dello schema motorio attraverso un confronto tra risultato atteso e risultato reale dell’azione
- [ ] D. È possibile una riformulazione del progetto d’azione senza alcuna verifica del risultato

## 766 - Metodologia

**Nella resistenza aerobica l’intensità dell’andatura è**

- [x] A. Molto blanda
- [ ] B. Intorno al massimo consumo di ossigeno
- [ ] C. Tolleranza al lattato
- [ ] D. Velocità submassimale

## 767 - Metodologia

**Nella scuola nuoto è corretto far eseguire esercizi di**

- [x] A. Forza rapida
- [ ] B. Resistenza alla forza
- [ ] C. Resistenza rapida
- [ ] D. Resistenza alla velocità

## 768 - Metodologia

**Nella tecnica del dorso la fase di spinta della bracciata termina**

- [x] A. Verso il basso
- [ ] B. Verso l’alto
- [ ] C. Verso l’esterno
- [ ] D. Contro il corpo

## 769 - Metodologia

**Nelle esercitazioni definite di potenza aerobica la frequenza cardiaca è**

- [ ] A. 130 - 150 Battiti per minuto
- [x] B. 170 - 190 Battiti per minuto
- [ ] C. 150 - 170 Battiti per minuto
- [ ] D. Oltre 190 battiti per minuto

## 770 - Metodologia

**Nello stile libero la fase di spinta dell’arto superiore avviene**

- [ ] A. Dopo la fase aerea
- [ ] B. Dopo l’appoggio-presa
- [ ] C. Prima della trazione
- [x] D. Dopo la trazione

## 771 - Metodologia

**Nuotare con gli occhi chiusi è un esercizio coordinativo di che tipo?**

- [x] A. Di esclusione
- [ ] B. Di accoppiamento
- [ ] C. Di differenziazione
- [ ] D. Di pronazione

## 772 - Metodologia

**Nuotare per brevi tratti molto veloci alternati a tratti lenti è un’attività di tipo**

- [ ] A. Aerobico
- [x] B. Anaerobico alattacido
- [ ] C. Anaerobico lattacido
- [ ] D. Tecnico-coordinativo

## 773 - Metodologia

**Parametri fisiologici delle esercitazioni A1-A2**

- [x] A. Frequenza cardiaca 150-180 bpm – lattacidemia 2,5-3 mmol/l
- [ ] B. Frequenza cardiaca < 150 bpm - lattacidemia < 2,5 mmol/l
- [ ] C. Frequenza cardiaca massimale – lattacidemia massimale
- [ ] D. Frequenza cardiaca > 180 bpm – lattacidemia > 2,5 mmol/l

## 774 - Metodologia

**Per allenare la forza rapida gli sforzi devono essere**

- [x] A. Brevissimi (3-9 secondi)
- [ ] B. Brevi (10-15 secondi)
- [ ] C. Medi (18-30 secondi)
- [ ] D. Lunghi (oltre il minuto)

## 775 - Metodologia

**Per allenare la resistenza bisogna**

- [ ] A. Nuotare velocemente per lunghi tratti
- [ ] B. Nuotare velocemente per brevi tratti
- [ ] C. Nuotare lentamente per brevi tratti
- [x] D. Nuotare lentamente per lunghi tratti

## 776 - Metodologia

**Per carico fisico esterno, intendiamo**

- [ ] A. La frequenza cardiaca mantenuta in allenamento
- [ ] B. La quantità di lavoro che si svolge all’aperto
- [ ] C. La massima sopportazione allo sforzo dell’organismo
- [x] D. La quantità degli esercizi di allenamento

## 777 - Metodologia

**Per fattori di prestazione s’intendono**

- [x] A. L’insieme delle capacità generali dell’individuo
- [ ] B. Le risorse interne dell’individuo
- [ ] C. I principi dell’allenamento
- [ ] D. La costituzione, la condizione, la coordinazione e la mobilità articolare

## 778 - Metodologia

**Per formare la struttura di base del nuotatore bisogna curare**

- [ ] A. L’affinamento del gesto
- [ ] B. Le esercitazioni analitiche
- [ ] C. Il benessere e il buonumore del gruppo
- [x] D. Il ritmo e l’ampiezza del gesto

## 779 - Metodologia

**Per incrementare il carico si varia prima**

- [ ] A. Il volume e l’intensità
- [x] B. La durata ed il volume
- [ ] C. L’intensità e la densità
- [ ] D. La durata e la densità

## 780 - Metodologia

**Per lo sviluppo della sensopercezione è utile**

- [x] A. Amplificare o inibire le informazioni provenienti dagli analizzatori
- [ ] B. Esercitarsi sulle singole azioni delle nuotate
- [ ] C. Stimolare i differenti meccanismi energetici
- [ ] D. Curare al massimo la comunicazione non verbale

## 781 - Metodologia

**Per lo sviluppo della sensopercezione è utile**

- [x] A. Amplificare o inibire le informazioni provenienti dagli analizzatori
- [ ] B. Esercitarsi sulle singole azioni delle nuotate
- [ ] C. Aumentare le ripetute
- [ ] D. Utilizzare un’ampia gamma di esercizi

## 782 - Metodologia

**Per provocare nuovi adattamenti a livello più elevato, un carico di lavoro deve**

- [ ] A. Sviluppare sempre il suo massimale
- [ ] B. Stabilizzarsi nel tempo
- [x] C. Progressivamente crescere
- [ ] D. Decrescere progressivamente

## 783 - Metodologia

**Per unità di base del movimento intendiamo**

- [ ] A. Le capacità condizionali
- [x] B. Gli schemi motori di base
- [ ] C. Le fasi delle bracciate nei diversi stili
- [ ] D. Le capacità coordinative

## 784 - Metodologia

**Perché lo stile libero è la nuotata più veloce?**

- [ ] A. Perché si possono fare la virate a capovolta senza toccare con le mani
- [x] B. Perché è la nuotata in cui il nuotatore può applicare la massima potenza angolare con un azione propulsiva continua
- [ ] C. Perché l’azione propulsiva delle gambe è particolarmente efficace
- [ ] D. Perché a differenza che negli altri stili, si nuota sempre in superficie

## 785 - Metodologia

**Qual è il momento di maggiore difficoltà nella virata a dorso?**

- [ ] A. Uscita dalla fase subacquea
- [ ] B. Rotazione al muro
- [x] C. Avvicinamento al muro e calcolo della giusta distanza
- [ ] D. Nessuna difficoltà particolare perché è una virata molto facile da eseguire

## 786 - Metodologia

**Qual è il presupposto biologico fondamentale per lo sviluppo della coordinazione?**

- [ ] A. La maturazione sessuale
- [ ] B. Lo sviluppo delle masse muscolari
- [ ] C. L’incremento della funzionalità cardiocircolatoria
- [x] D. La funzionalità del sistema nervoso centrale

## 787 - Metodologia

**Qual è l’allenamento tipico del periodo che va dall’età prepuberale all’adolescenza?**

- [ ] A. Di formazione
- [x] B. Di costruzione
- [ ] C. Di prestazione
- [ ] D. Di controllo

## 788 - Metodologia

**Qual è l’analizzatore sensoriale che invia al SNC le informazioni sulle tensioni muscolari?**

- [ ] A. Labirintico
- [ ] B. Vestibolare
- [x] C. Cinestetico
- [ ] D. Tattile

## 789 - Metodologia

**Qual è l’articolazione più sollecitata nel nuoto?**

- [ ] A. Gomito
- [ ] B. Anca
- [ ] C. Ginocchio
- [x] D. Spalla

## 790 - Metodologia

**Qual è l’età cronologica in cui gli stimoli per la velocità, o rapidità, possono avere maggiore efficacia?**

- [ ] A. Tra i 2 ed i 4 anni
- [ ] B. Tra i 3 ed i 6 anni
- [x] C. Tra i 6 ed i 13/14anni
- [ ] D. Dopo i 14 anni

## 791 - Metodologia

**Qual è la fascia di età ottimale per l’allenamento delle capacità coordinative?**

- [x] A. 6 - 12 anni
- [ ] B. Dai 12 ani in su
- [ ] C. 3 - 6 anni
- [ ] D. 0 - 6 anni

## 792 - Metodologia

**Qual è la fase più propulsiva della gambata a rana?**

- [ ] A. La fase di chiusura
- [x] B. L’azione calciata
- [ ] C. Entrambe perché continue
- [ ] D. La fase di chiusura e riallineamento

## 793 - Metodologia

**Qual è la frequenza cardiaca da raggiungere per allenare le capacità condizionali?**

- [ ] A. 120 battiti/minuto
- [ ] B. La massimale (220-età)
- [x] C. 70% - 90% della frequenza cardiaca massimale
- [ ] D. Non più del 70% della frequenza cardiaca massimale

## 794 - Metodologia

**Qual è la giusta definizione di abilità motoria?**

- [ ] A. La capacità del soggetto di eseguire azioni motorie complesse che necessitano di una certa dose di forza
- [x] B. Un’azione motoria automatizzata tramite ripetizioni consapevoli, che può essere eseguita senza l’intervento cosciente dell’allievo
- [ ] C. La possibilità dell’individuo di creare strategie motorie per la realizzazione dello scopo prefissato
- [ ] D. Uno schema motorio di base

## 795 - Metodologia

**Qual è la temperatura dell’acqua migliore per attività motorie intense e prolungate? #####**

- [x] A. 27 - 28° C #####
- [ ] B. 30 - 31° C #####
- [ ] C. 32 - 33° C #####
- [ ] D. 34 - 35° C

## 796 - Metodologia

**Quale combustibile viene prevalentemente utilizzato dal meccanismo anaerobico lattacido?**

- [x] A. Zuccheri
- [ ] B. Proteine
- [ ] C. Grassi
- [ ] D. Vitamine

## 797 - Metodologia

**Quale dei seguenti non è un criterio di selezione dei talenti da avviare all’attività agonistica?**

- [ ] A. Parametro ereditario (caratteristiche morfologiche dei genitori)
- [ ] B. Livello di sviluppo delle capacità coordinative e dell’immagine motoria
- [ ] C. Fattori psicologici e motivazionali
- [x] D. Primato personale sui 25 metri stile libero

## 798 - Metodologia

**Quale delle seguenti affermazioni è corretta?**

- [ ] A. Le capacità coordinative si sviluppano maggiormente fra quattro e sei anni
- [ ] B. Le capacità condizionali non mostrano differenze fra i sessi
- [x] C. Le capacità condizionali sono sempre sensibili all’allenamento, anche generale
- [ ] D. Le capacità coordinative sono forza, resistenza e velocità

## 799 - Metodologia

**Quale delle seguenti affermazioni sulla supercompensazione è corretta?**

- [ ] A. Determina un adattamento che si conclude con l’applicazione dello stimolo
- [x] B. Necessita di un tempo adeguato per potersi esprimere
- [ ] C. Necessita di un uno stimolo non necessariamente intenso
- [ ] D. Gli adattamenti sono permanenti

## 800 - Metodologia

**Quale delle seguenti affermazioni sulle virate non è corretta?**

- [x] A. Si costruiscono su abilità motorie semplici che hanno le loro fondamenta nella fase di ambientamento
- [ ] B. Vanno affrontate esclusivamente nei gruppi evoluti perché troppo difficili da apprendere nei gruppi intermedi di scuola nuoto
- [ ] C. In ogni livello di scuola nuoto vengono allenate, anche se non nella forma completa, comunque nei singoli elementi che le compongono
- [ ] D. Nella scuola nuoto è inutile insegnare le virate in quanto sono un’abilità tipica del nuoto agonistico

## 801 - Metodologia

**Quale delle seguenti è una capacità coordinativa generale?**

- [ ] A. Di equilibrio
- [ ] B. Di orientamento
- [x] C. Di controllo
- [ ] D. Di reazione

## 802 - Metodologia

**Quale delle seguenti può essere una causa di errore?**

- [ ] A. Fattori cinematici
- [ ] B. Dinamica del gesto
- [ ] C. Coordinazione inter- ed intramuscolare
- [x] D. Tutte le risposte precedenti

## 803 - Metodologia

**Quale di questi elementi incide maggiormente nella mancata coordinazione della nuotata a farfalla?**

- [ ] A. Recupero lento delle braccia
- [x] B. Discontinuità della bracciata
- [ ] C. Doppio colpo di gambe
- [ ] D. Nessuno dei precedenti

## 804 - Metodologia

**Quale di questi errori è più grave, e quindi va corretto prima?**

- [ ] A. Un errore di traiettoria della mano sott’acqua
- [ ] B. Una battuta di gambe deficitaria
- [x] C. Un errore di ritmo respiratorio
- [ ] D. Un errore nel recupero

## 805 - Metodologia

**Quale di questi particolari rende del tutto inefficace il colpo di gambe a rana?**

- [ ] A. Ginocchia troppo strette
- [x] B. Piedi fuori dall’acqua
- [ ] C. Gambe eccessivamente larghe
- [ ] D. Discontinuità del movimento

## 806 - Metodologia

**Quale è la prima fase da rispettare per un corretto processo di apprendimento motorio?**

- [x] A. Comunicazione e comprensione del gesto da effettuare
- [ ] B. Automatizzazione del gesto
- [ ] C. Intimorire gli allievi
- [ ] D. Sviluppi della coordinazione fine

## 807 - Metodologia

**Quale fascia di età è ritenuta la migliore per lo sviluppo delle capacità coordinative?**

- [x] A. Dai 5/6 ai 10/11 anni
- [ ] B. Dai 14 ai 18 anni
- [ ] C. Dai 2 ai 4 anni
- [ ] D. Dai 20 ai 30 anni

## 808 - Metodologia

**Quale fra i seguenti non è uno schema motorio di base?**

- [ ] A. Strisciare
- [ ] B. Rotolare
- [x] C. Galleggiare
- [ ] D. Arrampicare

## 809 - Metodologia

**Quale fra queste affermazioni sul metabolismo aerobico è errata?**

- [ ] A. È l’ultimo metabolismo che entra in funzione
- [ ] B. Permette performance di lunga durata e bassa intensità
- [x] C. Produce grosse quantità di acido lattico
- [ ] D. Richiede la presenza dell’ossigeno

## 810 - Metodologia

**Quale tra i seguenti non è un metodo didattico?**

- [ ] A. Globale
- [x] B. Grezzo
- [ ] C. Analitico
- [ ] D. Analitico Segmentario

## 811 - Metodologia

**Quale tra le seguenti affermazioni è corretta?**

- [x] A. Le abilità motorie sono lo sviluppo delle capacità motorie
- [ ] B. Capacità motorie ed abilità motorie non definiscono il talento
- [ ] C. Le capacità motorie sono prerequisiti del movimento
- [ ] D. Le abilità motorie sono il presuopposto delle capacità motorie

## 812 - Metodologia

**Quale tra le seguenti è una capacità condizionale?**

- [ ] A. Il ritmo della bracciata
- [ ] B. L’orientamento
- [x] C. La velocità
- [ ] D. Il galleggiamento

## 813 - Metodologia

**Quale tra le seguenti è una capacità condizionale?**

- [ ] A. La resistenza all’orientamento
- [ ] B. Il ritmo della bracciata
- [x] C. La forza resistente
- [ ] D. Il galleggiamento

## 814 - Metodologia

**Quale tra le seguenti è una capacità coordinativa generale?**

- [ ] A. Capacità di equilibrio
- [x] B. Capacità di adattamento e trasformazione
- [ ] C. Capacità di ritmizzazione
- [ ] D. Capacità di comprensione

## 815 - Metodologia

**Quale tra queste è la capacità coordinativa più importante per il nuoto?**

- [ ] A. Capacità di reazione
- [x] B. Capacità cinestesica
- [ ] C. Ritmo
- [ ] D. Orientamento

## 816 - Metodologia

**Quale tra questi analizzatori ha il ruolo di maggiore importanza nel nuoto?**

- [ ] A. Acustico
- [ ] B. Tattile
- [x] C. Cinestetico
- [ ] D. Ottico

## 817 - Metodologia

**Quale tra questi particolari va corretto prioritariamente?**

- [ ] A. La battuta di gambe
- [x] B. Le pause ed il ritmo
- [ ] C. L’ingresso della mano in acqua
- [ ] D. Il rollio

## 818 - Metodologia

**Quale, tra i seguenti, non è un meccanismo di produzione dell’energia?**

- [x] A. Aerobico glicolitico
- [ ] B. Aerobico
- [ ] C. Anaerobico alattacido
- [ ] D. Anaerobico lattacido

## 819 - Metodologia

**Quale, tra le seguenti, è una capacità coordinativa generale?**

- [ ] A. Capacità di equilibrio
- [ ] B. Capacita di controllo segmentario
- [ ] C. Capacità di orientamento
- [x] D. Capacità di direzione e controllo motorio

## 820 - Metodologia

**Quale, tra le seguenti, non è un’espressione della forza?**

- [ ] A. Forza massima
- [x] B. Forza coordinata
- [ ] C. Forza resistente
- [ ] D. Forza rapida

## 821 - Metodologia

**Quali dei seguenti elementi non fanno parte degli analizzatori?**

- [ ] A. Recettori specifici
- [ ] B. Vie nervose afferenti
- [ ] C. Centri sensoriali
- [x] D. Miofibrille

## 822 - Metodologia

**Quali di questi sport sono di coordinazione a mappa rigida?**

- [ ] A. Calcio – Ciclismo – Pallavolo
- [ ] B. Corsa - Pugilato – Pallacanestro
- [x] C. Ciclismo – Nuoto – Canottaggio
- [ ] D. Pallavolo – Pallacanestro – Nuoto

## 823 - Metodologia

**Quali metodi sono efficaci per lo sviluppo della forza rapida?**

- [ ] A. Sforzi di intensità medio-bassa, durata adeguata, pause brevi
- [x] B. Sforzi coscienti ad elevata intensità
- [ ] C. Sovraccarichi
- [ ] D. Training autogeno

## 824 - Metodologia

**Quali sfere sono investite dalla supercompensazione?**

- [x] A. Biologica, vegetativa, coordinativa
- [ ] B. Biologica, psicologica, pedagogica
- [ ] C. Psicologica, vegetativa, coordinativa
- [ ] D. Vegetativa, coordinativa, etimologica

## 825 - Metodologia

**Quali sono gli analizzatori coinvolti nel movimento?**

- [ ] A. Muscolare, scheletrico, articolare
- [ ] B. Cardiocircolatorio e respiratorio
- [x] C. Cinestesico, stato dinamico, tattile, visivo, uditivo
- [ ] D. Cinestesico e stato dinamico

## 826 - Metodologia

**Quali sono gli analizzatori?**

- [ ] A. Acustico, ottico, corporale, cinestesico, interno
- [x] B. Acustico, ottico, tattile, vestibolare, cinestesico
- [ ] C. Acustico, vestibolare, tattile, esterno, corporale
- [ ] D. Statico, dinamico

## 827 - Metodologia

**Quali sono gli aspetti strutturali di una nuotata?**

- [x] A. Continuità, ampiezza, ritmo
- [ ] B. Forza, velocità, resistenza
- [ ] C. Capacità coordinative generali e speciali
- [ ] D. Condizione, coordinazione, controllo

## 828 - Metodologia

**Quali sono gli strumenti per effettuare la selezione dei talenti?**

- [ ] A. Considerazioni del genitore
- [x] B. Gare sociali, istruttori e allenatori
- [ ] C. Desideri personali
- [ ] D. Consiglio dei compagni

## 829 - Metodologia

**Quali sono i fattori della prestazione sportiva?**

- [ ] A. Forza – resistenza – velocità
- [x] B. Costituzione - condizione - coordinazione – controllo
- [ ] C. Capacità condizionali e coordinative (generali e speciali)
- [ ] D. Antropometria – motivazione – disponibilità

## 830 - Metodologia

**Quali sono i parametri del carico?**

- [ ] A. Volume, durata, densità, intensità e multilateralità
- [ ] B. Durata, densità, intensità e multilateralità
- [ ] C. Costituzione, condizione, coordinazione, controllo
- [x] D. Volume, durata, densità e intensità

## 831 - Metodologia

**Quali sono i requisiti fondamentali delle nuotate elementari?**

- [x] A. Movimenti fini
- [ ] B. Ritmo e variazioni di velocità
- [ ] C. Rapidità di spostamento
- [ ] D. Continuità delle azioni ed ampiezza del gesto

## 832 - Metodologia

**Quali sono le capacità condizionali?**

- [ ] A. Resistenza, forza, volume
- [ ] B. Capacità motorie speciali
- [ ] C. Forza, resistenza, intensità
- [x] D. Forza, resistenza, velocità

## 833 - Metodologia

**Quali sono le capacità coordinative generali?**

- [ ] A. Ambientamento motorio, apprendimento motorio, controllo motorio
- [x] B. Apprendimento motorio, controllo motorio, adattamento e trasformazione dei movimenti
- [ ] C. Costituzione, condizione, coordinazione, controllo Apprendimento motorio generale, analitico motorio e segmentario

## 834 - Metodologia

**Quali sono le condizioni dell’analizzatore uditivo in acqua?**

- [ ] A. Riduzione delle capacità, impossibilità di controllare lo spazio d’azione
- [ ] B. Aumento delle quantità di stimolazione
- [ ] C. Invariate rispetto alla terraferma
- [x] D. Riduzione delle capacità

## 835 - Metodologia

**Quali sono le condizioni dell’analizzatore visivo in acqua?**

- [x] A. Riduzione delle capacità, impossibilità di controllare lo spazio d’azione
- [ ] B. Aumento delle quantità di stimolazione
- [ ] C. Invariate rispetto alla terraferma
- [ ] D. Riduzione delle capacità

## 836 - Metodologia

**Quali sono le condizioni più opportune per lo sviluppo delle capacità coordinative?**

- [ ] A. Eseguire fondamentali di uno sport in forma precisa
- [ ] B. Insistere solo sullo sviluppo dell’equilibrio
- [ ] C. Ripetere un movimento identico, numerose volte
- [x] D. Apprendimento di abilità motorie diverse

## 837 - Metodologia

**Quali sono le evoluzioni fisiologiche che differenziano un allenamento di contenuto aerobico da un allenamento di sforzo più intenso?**

- [ ] A. Aumento della frequenza cardiaca e incremento dell’acido lattico
- [x] B. Maggior intervento delle fibre rosse
- [ ] C. Aumento della frequenza cardiaca e nessun accumulo di acido lattico
- [ ] D. Situazione di equilibrio tra produzione e smaltimento di acido lattico

## 838 - Metodologia

**Quali sono le informazioni che raccogliamo dall’analizzatore cinestesico?**

- [ ] A. Pressione e calore
- [ ] B. Onde sonore
- [x] C. Posizione di una parte del corpo rispetto ad un’altra e stato di tensione di un muscolo
- [ ] D. Posizione della testa rispetto al centro di gravità

## 839 - Metodologia

**Quali sono le informazioni che raccogliamo dall’analizzatore statico-dinamico?**

- [ ] A. Onde sonore
- [ ] B. Segnali luminosi
- [ ] C. Posizione di una parte del corpo rispetto ad un’altra e stato di tensione di un muscolo
- [x] D. Posizione della testa rispetto al centro di gravità

## 840 - Metodologia

**Quali sono le informazioni che raccogliamo dall’analizzatore tattile?**

- [ ] A. Onde sonore
- [ ] B. Segnali luminosi
- [x] C. Pressione e calore
- [ ] D. Posizione di una parte del corpo rispetto a un’altra e stato di tensione di un muscolo

## 841 - Metodologia

**Quali sono le informazioni che raccogliamo dall’analizzatore uditivo?**

- [ ] A. Segnali luminosi
- [ ] B. Pressione e calore
- [x] C. Onde sonore
- [ ] D. Posizione della testa rispetto al centro di gravità

## 842 - Metodologia

**Quali sono le informazioni che raccogliamo dall’analizzatore visivo?**

- [x] A. Segnali luminosi
- [ ] B. Onde sonore
- [ ] C. Pressione e calore
- [ ] D. Sapori e odori

## 843 - Metodologia

**Quali sono le tappe dello sviluppo dello schema corporeo?**

- [ ] A. Infanzia, fanciullezza, giovinezza, età adulta
- [x] B. Corpo subìto, corpo vissuto, corpo percepito, corpo rappresentato
- [ ] C. Corpo amato, corpo percepito, corpo rappresentato
- [ ] D. Corpo bambino, corpo adolescente, corpo adulto

## 844 - Metodologia

**Quali tra i seguenti errori relativi ai quattro stili vanno corretti per primi?**

- [x] A. Errori di respirazione
- [ ] B. Errori di traiettoria
- [ ] C. Errori di ritmo
- [ ] D. Errori di ampiezza

## 845 - Metodologia

**Quali tra i seguenti sono schemi motori di base?**

- [ ] A. Flettere, circondurre, estendere
- [x] B. Rotolare, arrampicarsi, correre
- [ ] C. Contrarre, distendere
- [ ] D. Organizzare produrre, valutare

## 846 - Metodologia

**Quali tra le seguenti può essere considerata una capacità condizionale?**

- [x] A. Il galleggiamento
- [ ] B. Il ritmo della bracciata
- [ ] C. La velocità di comprensione
- [ ] D. La forza resistente

## 847 - Metodologia

**Quali tra le seguenti, può essere considerata una capacità condizionale?**

- [ ] A. Il galleggiamento
- [ ] B. La prossemica
- [x] C. La rapidità
- [ ] D. La velocità di apprendimento

## 848 - Metodologia

**Quali tra questi è un criterio di selezione per l’attività preagonistica?**

- [ ] A. Conoscenza delle distanze di gara
- [x] B. Fattori psicologici e motivazionali
- [ ] C. Rapporto tra massa grassa e massa magra
- [ ] D. Senso tattico

## 849 - Metodologia

**Quali tra questi è uno schema motorio acquatico?**

- [x] A. Scivolamento supino
- [ ] B. Corsa
- [ ] C. Appoggio plantare
- [ ] D. Salto a piedi uniti

## 850 - Metodologia

**Quali tra questi sono schemi motori di base?**

- [x] A. Rotolare, arrampicarsi, correre
- [ ] B. Flettere, circondurre, estendere
- [ ] C. Organizzare, produrre, valutare
- [ ] D. Fare, far fare, essere

## 851 - Metodologia

**Quando abbiamo la certezza che un errore sia stato corretto?**

- [ ] A. Quando l’allievo esegue il corretto movimento a secco
- [ ] B. Quando guidiamo il movimento dell’allievo in acqua
- [x] C. Quando il movimento corretto diventa un gesto volontario da parte dell’allievo
- [ ] D. Mai

## 852 - Metodologia

**Quando è consigliabile far eseguire esercizi di coordinazione?**

- [ ] A. Sin dalla prima lezione in vasca alta
- [x] B. Quando le forme globali delle nuotate sono state ben apprese ed automatizzate
- [ ] C. Non appena gli allievi hanno acquisito le prime forme di propulsione
- [ ] D. Sempre

## 853 - Metodologia

**Quando vengono inseriti due colpi di gambe a farfalla all’interno di un ciclo di bracciata?**

- [ ] A. Uno alla fine della fase di recupero quando le mani entrano in acqua e uno subito dopo lo scivolamento in fase di presa
- [x] B. Uno alla fine della fase di recupero quando le mani entrano in acqua e uno nella fase di spinta
- [ ] C. Uno durante la fase di trazione e uno alla fine della fase di presa
- [ ] D. Uno dopo la fase di presa e uno dopo la fase di recupero quando le mani entrano in acqua

## 854 - Metodologia

**Quella di adattamento e trasformazione è**

- [ ] A. Un’attitudine individuale
- [ ] B. Una capacità condizionale non sempre allenabile
- [x] C. Una capacità coordinativa speciale
- [ ] D. Una capacità coordinativa generale

## 855 - Metodologia

**Sotto il profilo dell’organizzazione motoria, il principio cefalo-caudale insegna che lo sviluppo avviene rispettando quale subordinazione?**

- [ ] A. Gambe - busto - braccia - testa
- [x] B. Testa - busto - braccia - gambe
- [ ] C. Busto - gambe - braccia – testa
- [ ] D. Braccia – gambe – testa - busto

## 856 - Metodologia

**Strisciare è**

- [x] A. Uno schema motorio di base
- [ ] B. Una tecnica appresa
- [ ] C. Un’abilità motoria di secondo grado
- [ ] D. Uno schema motorio evoluto

## 857 - Metodologia

**Su quale base si effettua la scelta degli atleti?**

- [x] A. Capacità di apprendimento rapido
- [ ] B. Fattori sociali
- [ ] C. Fattori relazioni
- [ ] D. Capacità di ambientamento

## 858 - Metodologia

**Tra gli esercizi di specializzazione a dorso a cosa serve la doppia bracciata?**

- [ ] A. A tenere la testa ferma
- [ ] B. A enfatizzare il rollio e a velocizzare il recupero
- [ ] C. A migliorare la coordinazione con le gambe
- [x] D. Insegna la posizione del gomito alto e a enfatizzare la presa

## 859 - Metodologia

**Tra gli esercizi di specializzazione della farfalla a cosa serve utilizzo della gambata a stile libero?**

- [ ] A. A sviluppare i muscoli del grande tronco
- [ ] B. A enfatizzare il rollio e a velocizzare il recupero
- [ ] C. A migliorare la coordinazione tra le braccia e la testa
- [x] D. A ridurre le ondulazioni e a stimolare una bracciata senza pause

## 860 - Metodologia

**Tra i fattori della prestazione troviamo il controllo; cos’è?**

- [ ] A. La capacità di controllare e monitorare il proprio corpo e l’ambiente circostante
- [ ] B. Uno schema motorio di base
- [x] C. L’insieme dei processi emotivi, cognitivi e motivazionali
- [ ] D. L’insieme dei processi emotivi e cognitivi

## 861 - Metodologia

**Tra i parametri del carico fisico, cos’è la densità?**

- [ ] A. La quantità di sforzo che un organismo produce nell’eseguire un esercizio
- [x] B. Il comportamento temporale del tempo di stimolo e di riposo in una unità di allenamento
- [ ] C. Il livello dell’impegno richiesto al soggetto rispetto alle sue capacità massimali
- [ ] D. Un parametro che indica il comportamento evolutivo della frequenza di un particolare sforzo con il relativo adattamento

## 862 - Metodologia

**Tra quelle elencate quali sono le condizioni che favoriscono lo sviluppo delle capacità coordinative?**

- [ ] A. Che siano eseguite tecniche precise, scelte tra i fondamentali del nuoto
- [ ] B. Che gli esercizi proposti diventino immediatamente più difficili e più complicati
- [x] C. Che venga utilizzato un allenamento ricco di stimoli motori
- [ ] D. Che ogni esercizio venga ripetuto, identico, numerose volte

## 863 - Metodologia

**Un esercizio è non strutturato quando**

- [ ] A. Si definiscono tutti i dettagli dell’esecuzione
- [ ] B. Si definiscono alcuni aspetti dell’esecuzione e altri sono lasciati alla scelta dell’allievo
- [ ] C. Non prevede la fase di recupero
- [x] D. Non si richiede nessuna esecuzione ma si agisce sul contesto per stimolare l’azione dell’allievo

## 864 - Metodologia

**Un esercizio è semi-strutturato quando**

- [ ] A. Si definiscono tutti i dettagli dell’esecuzione
- [ ] B. Prevede la fase di recupero
- [x] C. Si definiscono alcuni aspetti dell’esecuzione e altri sono lasciati alla scelta dell’allievo
- [ ] D. Non si richiede nessuna esecuzione ma si agisce sul contesto per stimolare l’azione dell’allievo

## 865 - Metodologia

**Un esercizio è strutturato quando**

- [x] A. Si definiscono tutti i dettagli dell’esecuzione
- [ ] B. Prevede la fase di recupero
- [ ] C. Si definiscono alcuni aspetti dell’esecuzione e altri sono lasciati alla scelta dell’allievo
- [ ] D. Non si richiede nessuna esecuzione ma si agisce sul contesto per stimolare l’azione dell’allievo

## 866 - Metodologia

**Un modello di movimento ben consolidato nella mente, facilmente riproducibile, è detto**

- [ ] A. Schema complesso
- [ ] B. Schema corporeo
- [ ] C. Schema tattico
- [x] D. Schema motorio

## 867 - Metodologia

**Un’elevata mobilità articolare comporta**

- [ ] A. Delle difficoltà nella fase di recupero aereo
- [ ] B. Una migliore sensibilità in acqua
- [x] C. Una nuotata più fluida ed economica
- [ ] D. Nessun vantaggio o svantaggio particolare
