# Psicopedagogia

## 868 - Psicopedagogia

**A che fascia di età corrisponde la seconda infanzia?**

- [ ] A. Oltre i 12 anni
- [ ] B. Da 5 a 18 mesi
- [ ] C. Da 7 a 12 anni
- [x] D. Da 2 a 6 anni

## 869 - Psicopedagogia

**A quali delle seguenti parole è collegabile la comunicazione non verbale?**

- [ ] A. Il gioco
- [x] B. La prossemica
- [ ] C. L’ansia da prestazione
- [ ] D. La leadership

## 870 - Psicopedagogia

**Caratteristiche di un bambino da 0 a 3 anni**

- [ ] A. Motricità volontaria
- [ ] B. Motricità involontaria
- [x] C. Strutturazione percettiva e coscienza del proprio corpo
- [ ] D. Immagine sintetica del corpo, rappresentazione decentrata, rappresentazione mentale

## 871 - Psicopedagogia

**Caratteristiche di un bambino da 3 a 7 anni**

- [x] A. Motricità volontaria
- [ ] B. Motricità involontaria
- [ ] C. Strutturazione percettiva e coscienza del proprio corpo
- [ ] D. Immagine sintetica del corpo, rappresentazione decentrata, rappresentazione mentale

## 872 - Psicopedagogia

**Caratteristiche di un bambino da 7 a 12 anni**

- [ ] A. Motricità volontaria
- [ ] B. Motricità involontaria
- [ ] C. Strutturazione percettiva e coscienza del proprio corpo
- [x] D. Immagine sintetica del corpo, rappresentazione decentrata, rappresentazione mentale

## 873 - Psicopedagogia

**Che cosa si deve intendere per frustrazione?**

- [ ] A. Un tratto caratteristico della personalità degli adolescenti
- [x] B. La percezione dell’impedimento a soddisfare bisogni e aspirazioni
- [ ] C. Il desiderio di soffrire
- [ ] D. L’autolesionismo

## 874 - Psicopedagogia

**Come minimizzare i fallimenti?**

- [x] A. Valutare il risultato, non l’autore
- [ ] B. Consolare direttamente l’allievo
- [ ] C. Riprendere immediatamente il lavoro dopo la crisi
- [ ] D. Ridurre al minimo le fasi di gioco

## 875 - Psicopedagogia

**Comunicare significa**

- [ ] A. Saper parlare bene
- [ ] B. Mimare
- [ ] C. Comprendere
- [x] D. Mettere in comune

## 876 - Psicopedagogia

**Cos’è il messaggio di contenuto?**

- [x] A. Ciò che l’emittente intende trasmettere rappresentato dal significato del codice utilizzato
- [ ] B. tutte quelle informazioni che arrivano al destinatario circa l’andamento della relazione tra i due comunicandi, espressi dal linguaggio del corpo
- [ ] C. ciò che l’emissario comprende del contenuto espresso dal codice
- [ ] D. Una figura retorica

## 877 - Psicopedagogia

**Cos’è il messaggio di relazione?**

- [ ] A. Ciò che l’emittente intende trasmettere rappresentato dal significato del codice utilizzato
- [ ] B. Una figura retorica
- [x] C. Tutte quelle informazioni che arrivano al destinatario circa l’andamento della relazione tra i due comunicandi, espressi dal linguaggio del corpo
- [ ] D. Ciò che l’emissario comprende del contenuto espresso dal codice

## 878 - Psicopedagogia

**Cos’è l’autostima?**

- [ ] A. Una valutazione personale
- [ ] B. L’incoraggiamento dell’istruttore
- [x] C. La considerazione che un individuo ha di sé stesso
- [ ] D. L’uso della lode come rinforzo positivo

## 879 - Psicopedagogia

**Cos’è un feedback?**

- [x] A. Un’informazione di ritorno
- [ ] B. Un processo di controllo motorio
- [ ] C. Un metodo di comunicazione
- [ ] D. Un supporto didattico

## 880 - Psicopedagogia

**Cosa s’intende per autostima?**

- [ ] A. Il processo soggettivo e duraturo che porta il soggetto a apprezzare se stesso tramite la decisione di valorizzare se stesso
- [ ] B. Il livello di soddisfazione percepito
- [x] C. Il processo soggettivo e duraturo che porta il soggetto a valutare e apprezzare se stesso tramite l’auto approvazione del proprio valore personale, fondato su quello che uno vede di sé
- [ ] D. Il processo soggettivo che porta un soggetto alla miglior prestazione possibile nella gara obiettivo

## 881 - Psicopedagogia

**Cosa s’intende per linguaggio del corpo?**

- [ ] A. Postura, gesti, movimenti, espressioni e mimica che contraddicono la comunicazione verbale
- [x] B. Postura, gesti, movimenti, espressioni e mimica che accompagnano la parola e rendendo la comunicazione più comunicativa esprimendo il messaggio di relazione
- [ ] C. Postura, gesti, movimenti, espressioni e mimica che accompagnano la parola e rendendo la comunicazione incomprensibile
- [ ] D. L’insieme delle capacità coordinative e condizionali

## 882 - Psicopedagogia

**Cosa s’intende per linguaggio?**

- [ ] A. La lingua che si usa nella comunicazione
- [x] B. Il sistema di segni articolato da una grammatica e una sintassi che consente la comunicazione
- [ ] C. Il sistema di segni con cui si rende più efficiente una comunicazione
- [ ] D. L’inflessione linguistica

## 883 - Psicopedagogia

**Cosa significa comportamento modulare dell’istruttore?**

- [x] A. Comportarsi in base alle varie esigenze, considerando il ruolo di educatore
- [ ] B. Essere autoritari
- [ ] C. Essere permissivi
- [ ] D. Essere indifferenti

## 884 - Psicopedagogia

**Da cosa è caratterizzato il processo di sviluppo?**

- [ ] A. Dalla successione di fasi di accelerazione/decelerazione
- [ ] B. Dalla presenza di sole fasi di accelerazione seguite da ristrutturazione cognitiva, motoria, affettiva
- [x] C. Dall’alternanza di fasi di accelerazione/decelerazione e crisi
- [ ] D. Dall’assenza di variabilità individuale

## 885 - Psicopedagogia

**Da cosa origina il concetto di unità psicofisica?**

- [ ] A. Emerge dalla constatazione che il corpo è un oggetto a stretto contatto con la psiche
- [x] B. Emerge dalla constatazione che il corpo umano non è mai corpo in senso oggettivo e la psiche non ha alcun senso se non si fonda e si costituisce attraverso e insieme al corpo
- [ ] C. Emerge dalla constatazione che la psiche è l’elemento che differenzia e determina ogni essere umano
- [ ] D. Dagli studi di Meinel

## 886 - Psicopedagogia

**Definizione di canale**

- [ ] A. Il contenuto della comunicazione
- [ ] B. Il sistema di segni usato per comunicare
- [ ] C. La scelta del registro da utilizzare
- [x] D. Il mezzo fisico attraversato dalla comunicazione

## 887 - Psicopedagogia

**Definizione di codice**

- [ ] A. La fonte del messaggio. Nel caso di comunicazione umana è il soggetto che intende comunicare
- [x] B. Il sistema di segni usato per comunicare
- [ ] C. Il mezzo fisico attraversato dalla comunicazione
- [ ] D. Raccolta di abbreviazioni utili per comunicare sinteticamente

## 888 - Psicopedagogia

**Definizione di contesto**

- [ ] A. La fonte del messaggio
- [ ] B. Codice di riferimento
- [ ] C. Il sistema di segni usato per comunicare
- [x] D. La situazione che rende comprensibile una comunicazione

## 889 - Psicopedagogia

**Definizione di destinatario**

- [ ] A. Il contenuto della comunicazione
- [ ] B. Beneficiario della comunicazione
- [x] C. Colui al quale è indirizzato il messaggio
- [ ] D. Il sistema di segni usato per comunicare

## 890 - Psicopedagogia

**Definizione di emittente**

- [x] A. La fonte del messaggio
- [ ] B. Colui al quale è indirizzato il messaggio
- [ ] C. Il contenuto della comunicazione
- [ ] D. Il sistema di segni usato per comunicare

## 891 - Psicopedagogia

**Definizione di messaggio**

- [x] A. Il contenuto della comunicazione
- [ ] B. Il sistema di segni usato per comunicare
- [ ] C. Registro
- [ ] D. La situazione che rende comprensibile una comunicazione

## 892 - Psicopedagogia

**È una forma di apprendimento fondamentale**

- [x] A. Apprendimento per imitazione
- [ ] B. Apprendimento per deduzione
- [ ] C. Apprendimento per interpretazione
- [ ] D. Apprendimento per costrizione

## 893 - Psicopedagogia

**Elementi fondamentali per la strutturazione di un adeguato metodo di lavoro sono**

- [ ] A. Atteggiamento ipercritico da parte dell’insegnante
- [ ] B. Ricerca di assoluta precisione nell’apprendimento del particolare specifico
- [ ] C. Casualità degli interventi didattici
- [x] D. Globalità dell’apprendimento, clima affettivo/ relazionale positivo, gestione del gruppo

## 894 - Psicopedagogia

**Il bambino, per adattarsi alle varie situazioni del mondo esterno, deve avere**

- [ ] A. Un ambiente che lo stimoli
- [ ] B. Una mente olistica
- [x] C. Coscienza e conoscenza del proprio corpo
- [ ] D. Dei bravi genitori

## 895 - Psicopedagogia

**Il concetto di apprendimento è strettamente collegato al concetto di**

- [x] A. Motivazione
- [ ] B. Memorizzazione
- [ ] C. Intelligenza
- [ ] D. Comprendonio

## 896 - Psicopedagogia

**Il concetto di sé è**

- [ ] A. La valutazione che si ha circa le svariate informazioni che riguardano il sé
- [x] B. L’insieme di tutti gli elementi a cui una persona fa riferimento per descrivere sé stessa
- [ ] C. Il complesso di tutte le valutazioni che una persona ha ricevuto sul proprio conto
- [ ] D. Una capacità coordinativa speciale

## 897 - Psicopedagogia

**Il linguaggio del corpo, i paralinguaggi fanno parte della**

- [ ] A. Comunicazione verbale
- [ ] B. Dinamica comunicativa
- [ ] C. Prossemica
- [x] D. Comunicazione non verbale

## 898 - Psicopedagogia

**Il messaggio è**

- [x] A. Il contenuto della comunicazione
- [ ] B. Un sistema di simboli
- [ ] C. Colui che avvia la comunicazione
- [ ] D. Colui che riceve la comunicazione

## 899 - Psicopedagogia

**Il paralinguaggio mostra**

- [ ] A. Il ceto sociale
- [x] B. Gli stati emotivi o affettivi
- [ ] C. L’espressività della persona
- [ ] D. Il grado di sviluppo coordinativo

## 900 - Psicopedagogia

**Il paralinguaggio**

- [ ] A. Non cambia il significato verbale di una frase
- [ ] B. In taluni soggetti non è presente
- [x] C. Accompagna l’espressione verbale
- [ ] D. Non dipende da stati emotivi o affettivi

## 901 - Psicopedagogia

**Il percorso didattico e le modalità relazionali devono essere**

- [ ] A. Oggetto di contrattazione fra istruttore e allievi
- [ ] B. Elementi della comunicazione
- [x] C. Determinati e gestiti dall’istruttore
- [ ] D. Affidati al caso

## 902 - Psicopedagogia

**Il principio guida dell’insegnamento è la convinzione che**

- [ ] A. Nessun allievo può realmente imparare
- [ ] B. Ciascun allievo deve fare da sé
- [ ] C. Non esistono convinzioni
- [x] D. Tutti gli allievi possono imparare

## 903 - Psicopedagogia

**Il raggiungimento di un obiettivo è legato al concetto di**

- [ ] A. Ambientamento
- [ ] B. Gareggiare
- [ ] C. Costituzione
- [x] D. Motivazione

## 904 - Psicopedagogia

**Il ruolo all’interno di un’organizzazione è**

- [ ] A. Il risultato delle attitudini e della loro applicazione, unite alla volontà di comportarsi in un dato modo e all’interesse per le conseguenze delle azioni che compie
- [ ] B. La tipologia contrattuale applicata
- [x] C. L’assunzione di responsabilità personale richiesta da una certa mansione e dai suoi obiettivi
- [ ] D. L’insieme organico dei compiti assegnati

## 905 - Psicopedagogia

**Il ruolo dell’istruttore è**

- [ ] A. Insegnare con scrupolo il gesto tecnico
- [x] B. Essere una figura di riferimento in vasca per i propri allievi
- [ ] C. Assistere alla sicurezza dei nuotatori
- [ ] D. Riflettere sul tutto

## 906 - Psicopedagogia

**Il tempo di apprendimento è dovuto a**

- [ ] A. Capacità personali
- [x] B. Capacità personali, desiderio di imparare, tempo di applicazione
- [ ] C. Bravura del maestro
- [ ] D. Durata della singola lezione

## 907 - Psicopedagogia

**In quale fase dello sviluppo si ha difficoltà a percepire correttamente il proprio corpo?**

- [ ] A. Seconda infanzia
- [ ] B. Fanciullezza
- [x] C. Preadolescenza e adolescenza
- [ ] D. Maturità

## 908 - Psicopedagogia

**In tema di apprendimento, quale di queste affermazioni è appropriata?**

- [ ] A. L’apprendimento dipende dalle capacità cognitive del soggetto
- [x] B. Non esiste atto cognitivo che prescinda o che non sia preceduto da una componente emotiva
- [ ] C. L’apprendimento dipende sicuramente da molti fattori,ma soprattutto dalla capacità dell’istruttore di gestire la disciplina all’interno del gruppo
- [ ] D. L’apprendimento non è quantificabile

## 909 - Psicopedagogia

**In un messaggio inviato, l’aspetto non verbale della comunicazione incide per**

- [ ] A. Circa il 10%
- [ ] B. Circa il 30%
- [ ] C. Circa il 50%
- [x] D. Circa il 90%

## 910 - Psicopedagogia

**Indica quali effetti l’attività sportiva produce sulla personalità**

- [ ] A. Miglioramento dell’autostima e minore autonomia
- [x] B. Miglioramento dell’autostima e maggiore autonomia
- [ ] C. Senso di superiorità e narcisismo
- [ ] D. Tendenza alla competitività

## 911 - Psicopedagogia

**L’apprendimento è facilitato**

- [ ] A. Dalla qualità della comunicazione tra allievo e istruttore
- [x] B. Dalla motivazione
- [ ] C. Dalla famiglia
- [ ] D. Dalla genetica

## 912 - Psicopedagogia

**L’apprendimento è fortemente influenzato**

- [ ] A. Dall’ambiente della piscina
- [x] B. Dalla motivazione
- [ ] C. Dall’età
- [ ] D. Dal sesso

## 913 - Psicopedagogia

**L’apprendimento è**

- [x] A. L’attuazione consapevole di un principio didattico
- [ ] B. L’applicazione sistematica di un automatismo
- [ ] C. La consapevolezza dell’io corporeo
- [ ] D. L’applicazione di un esercizio dimostrato dall’istruttore

## 914 - Psicopedagogia

**L’apprendimento può essere definito come**

- [x] A. Un cambiamento di comportamento
- [ ] B. Un’acquisizione di conoscenze
- [ ] C. Una realtà oggettiva esterna
- [ ] D. L’insieme delle condizioni ambientali

## 915 - Psicopedagogia

**L’apprendimento si realizza attraverso**

- [ ] A. Casualità e causalità
- [x] B. Scoperta – conoscenza – consolidamento
- [ ] C. Conoscenza – scoperta – esecuzione
- [ ] D. Comunicazione – motivazione – esecuzione

## 916 - Psicopedagogia

**L’area affettivo-emotiva della personalità**

- [x] A. Riguarda l’insieme dei rapporti che si creano tra l’individuo e le persone che vivono con lui
- [ ] B. Riguarda il rapporto con i genitori
- [ ] C. Riguarda il senso di appartenenza e di coesione, lo spirito di emulazione e il rispetto degli altri.
- [ ] D. Riguarda la capacità di ricevere informazioni dall’esterno del proprio corpo ed elaborare delle risposte sotto forma di movimento

## 917 - Psicopedagogia

**L’aspetto realizzativo è legato al concetto di**

- [x] A. Motivazione
- [ ] B. Competizione
- [ ] C. Interdizione
- [ ] D. Sicurezza in acqua

## 918 - Psicopedagogia

**L’azione educativa del nuoto si concretizza attraverso**

- [ ] A. L’insegnamento dell’istruttore
- [x] B. L’educazione al nuoto e l’educazione attraverso il nuoto
- [ ] C. Il rispetto delle regole
- [ ] D. L’apprendimento degli stili

## 919 - Psicopedagogia

**L’empatia**

- [x] A. Agevola la comunicazione
- [ ] B. Non influisce sulla comunicazione
- [ ] C. Rallenta la comunicazione
- [ ] D. È una capacità coordinativa speciale

## 920 - Psicopedagogia

**L’intelligenza preconcettuale si sviluppa**

- [x] A. Dai 18 mesi ai 7 anni
- [ ] B. Verso i 7 anni
- [ ] C. Dopo i 7 anni
- [ ] D. Appena nati

## 921 - Psicopedagogia

**L’intelligenza sensomotoria si sviluppa**

- [ ] A. Appena nati
- [x] B. Durante i primi 18 mesi di vita
- [ ] C. Dopo i 18 mesi di vita
- [ ] D. In corrispondenza della pubertà

## 922 - Psicopedagogia

**L’istruttore assume un atteggiamento direttivo**

- [x] A. Quando si pone in posizione di diversità rispetto al gruppo e ne assume la conduzione
- [ ] B. Quando applica una logica sfumata (fuzzy)
- [ ] C. Quando si pone in condizione di vicinanza con il gruppo ma ne assume la leadership
- [ ] D. Quando si pone in condizione di parità e interagisce con il gruppo

## 923 - Psicopedagogia

**L’istruttore assume un atteggiamento non direttivo**

- [ ] A. Quando si pone in posizione di diversità rispetto al gruppo e ne assume la conduzione
- [ ] B. Quando applica una logica sfumata (fuzzy)
- [ ] C. Quando si pone in condizione di vicinanza con il gruppo ma ne assume la leadership
- [x] D. Quando si pone in condizione di parità e interagisce con il gruppo

## 924 - Psicopedagogia

**L’Istruttore nel rapporto con l’allievo**

- [x] A. Deve sapere, saper fare e saper far fare
- [ ] B. Deve umiliarlo
- [ ] C. Deve proporre esercizi adeguati
- [ ] D. Deve curare la tecnica

## 925 - Psicopedagogia

**L’istruttore**

- [ ] A. È il leader del gruppo
- [x] B. Utilizza la leadership per orientare il gruppo verso un obiettivo
- [ ] C. È parte integrante del gruppo
- [ ] D. Deve prestare attenzione ai soli aspetti tecnici

## 926 - Psicopedagogia

**L’obiettivo della programmazione di un percorso didattico deve essere**

- [ ] A. Irraggiungibile ed indipendente dal livello di partenza dell’allievo
- [ ] B. Raggiungibile per il tecnico ed indipendente dal livello di partenza dell’allievo
- [x] C. Ecologico per l’allievo e formulato in termini sensorialmente basati
- [ ] D. Ecologico per l’allievo e formulato in negativo

## 927 - Psicopedagogia

**La comunicazione a due vie è caratterizzata da**

- [x] A. Presenza di emittente, messaggio, ricevente e feed-back verbale o non verbale
- [ ] B. Possibilità di confronto verbale tra trasmittente e ricevente
- [ ] C. Utilizzo di strumenti ricetrasmittenti
- [ ] D. Verbosità e non verbosità

## 928 - Psicopedagogia

**La comunicazione è**

- [ ] A. Il passaggio di informazioni verbali tra due soggetti
- [ ] B. La capacità di esprimere i propri pensieri
- [ ] C. Un processo monocanale
- [x] D. Un processo attivo di elaborazione di informazioni, di codifiche e di decodifiche

## 929 - Psicopedagogia

**La comunicazione è**

- [x] A. Verbale e non verbale
- [ ] B. Digitale e analogica
- [ ] C. Corporea
- [ ] D. Prossimale o distale

## 930 - Psicopedagogia

**La comunicazione si divide in**

- [x] A. Verbale e non verbale
- [ ] B. Digitale e analogica
- [ ] C. Corporea e visiva
- [ ] D. Tattile e olfattiva

## 931 - Psicopedagogia

**La comunicazione verbale è**

- [ ] A. La capacità di saper informare nel modo più semplice l’allievo
- [x] B. La capacità di esprimere correttamente e adeguatamente il proprio pensiero attraverso il linguaggio
- [ ] C. La capacità di gestire e comandare il proprio corpo
- [ ] D. Una capacità coordinativa speciale

## 932 - Psicopedagogia

**La fanciullezza va**

- [ ] A. Oltre i 14 anni
- [ ] B. Dagli 11 ai 14 anni
- [ ] C. Dai 3 ai 6 anni
- [x] D. Dai 6 agli 11 anni

## 933 - Psicopedagogia

**La figura del leader all’interno del gruppo di lavoro è**

- [ ] A. Una figura scomoda, che mette i bastoni tra le ruote a tutto quello che cerchiamo di fare durante l’insegnamento, va combattuta e indebolita
- [x] B. Colui che esprime una personalità carismatica in grado di soddisfare le esigenze di tutti i membri del gruppo
- [ ] C. Un modello di vita
- [ ] D. Una figura carismatica, in genere il più dotato dal punto di vista prestazionale, pertanto sarà il soggetto del nostro interesse previlegiato

## 934 - Psicopedagogia

**La forma d’insegnamento più efficace è**

- [ ] A. Stimolare l’allievo a riprodurre fedelmente la tecnica di nuotata del suo istruttore
- [ ] B. Far ripetere all’allievo, fino alla meccanizzazione, il movimento in acqua
- [ ] C. Correggere gli errori del movimento appena si manifestano
- [x] D. Stimolare l’allievo ad una riflessione cosciente e continua sul proprio movimento

## 935 - Psicopedagogia

**La motivazione è**

- [ ] A. Il bisogno di muoversi e fare sport
- [ ] B. Ciò che spinge un individuo ad imparare a nuotare
- [x] C. L’insieme dei fattori che avviano e indirizzano le azioni di un soggetto
- [ ] D. Necessaria per i bambini

## 936 - Psicopedagogia

**La motivazione intrinseca è**

- [ ] A. Lo stimolo a partecipare interno al soggetto e decade con rapidità
- [ ] B. Lo stimolo a partecipare esterno al soggetto e decade con difficoltà
- [x] C. Lo stimolo a partecipare interno al soggetto e decade con difficoltà
- [ ] D. Lo stimolo a partecipare dietro costrizione

## 937 - Psicopedagogia

**La motivazione**

- [ ] A. È legata all’età dell’individuo
- [ ] B. Dipende dal percorso filogenetico
- [x] C. È alla base dell’apprendimento
- [ ] D. Non conosce fattori inibenti

## 938 - Psicopedagogia

**La personalità viene definita come**

- [x] A. Una sintesi originale, dinamica e relativamente stabile dei modi di essere e di agire dell’individuo
- [ ] B. Lo sviluppo biologico che evidenzia i fenomeni di maturazione di organi ed apparati
- [ ] C. Il carattere
- [ ] D. La capacità di apprendimento

## 939 - Psicopedagogia

**La prima infanzia va**

- [x] A. Da 0 a 3 anni
- [ ] B. Da 3 a 6 anni
- [ ] C. Da 6 a 11 anni
- [ ] D. Dai 12 anni

## 940 - Psicopedagogia

**La prossemica studia**

- [x] A. La gestione dello spazio tra sé e gli altri
- [ ] B. I linguaggi del corpo
- [ ] C. L’utilizzo della voce e le sue modulazioni
- [ ] D. Il funzionamento della comunicazione verbale

## 941 - Psicopedagogia

**La prossemica**

- [ ] A. Accompagna l’espressione verbale come intensità, ritmo, la velocità e l’intonazione
- [x] B. Definisce il modo in cui una persona inconsapevolmente struttura lo spazio tra sé e gli altri nel corso delle interazioni sociali
- [ ] C. Riguarda la capacità di ricevere informazioni dall’esterno del proprio corpo ed elaborare delle risposte sotto forma di movimento.
- [ ] D. È una caratteristica del linguaggio verbale

## 942 - Psicopedagogia

**La prossemica**

- [ ] A. È inconsapevole perché tiene conto dei fattori motivazionali
- [ ] B. È consapevole
- [x] C. È inconsapevole
- [ ] D. È consapevole perché tiene conto dei fattori motivazionali

## 943 - Psicopedagogia

**La qualità del servizio nel lavoro dell’istruttore è data da**

- [ ] A. Capacità d’insegnamento e rispettabilità
- [x] B. Capacità di relazione e procedure che si adottano nello svolgere il lavoro
- [ ] C. Abbigliamento e portamento dell’istruttore
- [ ] D. Tempo di apprendimento delle abilità tecniche

## 944 - Psicopedagogia

**La seconda infanzia va**

- [x] A. Dai 3 ai 6 anni
- [ ] B. Prima dei 3 anni
- [ ] C. Dopo 6 anni
- [ ] D. Nessuna delle risposte precedenti

## 945 - Psicopedagogia

**La valutazione delle informazioni che derivano dal concetto di sé costituisce**

- [x] A. L’autostima
- [ ] B. La depressione
- [ ] C. La formazione
- [ ] D. L’aggiornamento

## 946 - Psicopedagogia

**Le aspettative che gli istruttori di nuoto nutrono nei confronti dei propri allievi**

- [x] A. Influenzano positivamente o negativamente l’ apprendimento
- [ ] B. Non hanno effetti sull’ apprendimento
- [ ] C. Influenzano l’apprendimento ma non in modo rilevante
- [ ] D. Sono pregiudizi che vanno combattuti

## 947 - Psicopedagogia

**Le dinamiche comunicative possono essere di tre livelli**

- [x] A. Comunicazione verbale, comunicazione non verbale, paralinguaggi
- [ ] B. Ansia, inferiorità, frustrazione
- [ ] C. Simboliche, olistiche, materiche
- [ ] D. Verbali, transferali, psicologiche

## 948 - Psicopedagogia

**Le forme fondamentali per l’apprendimento sono**

- [x] A. Apprendimento per imitazione, condizionamento, per prove ed errori, per intuizione, per comprensione
- [ ] B. Apprendimento per imitazione, condizionamento, per prove ed errori, per intuizione, per ricostruzione
- [ ] C. Apprendimento per determinazione
- [ ] D. Apprendimento per imitazione, condizionamento, per prove ed errori, per intuizione, per decifrazione

## 949 - Psicopedagogia

**Le mansioni in un’organizzazione corrispondono a**

- [ ] A. Il risultato delle attitudini e della loro applicazione, unite alla volontà di comportarsi in un dato modo e all’interesse per le conseguenze delle azioni che compie.
- [ ] B. La tipologia contrattuale applicata
- [ ] C. L’assunzione di responsabilità personale richiesta da una certa mansione e dai suoi obiettivi
- [x] D. L’insieme organico dei compiti assegnati

## 950 - Psicopedagogia

**Le motivazioni generate da necessità proprie dell’individuo si chiamano**

- [ ] A. Motivazioni estrinseche
- [ ] B. Motivazioni personali
- [ ] C. Motivazioni sociali
- [x] D. Motivazioni intrinseche

## 951 - Psicopedagogia

**Le motivazioni generate dall’ambiente che circonda l’individuo, si chiamano**

- [ ] A. Motivazioni intrinseche
- [ ] B. Pressioni esogene
- [x] C. Motivazioni sociali
- [ ] D. Motivazioni estrinseche

## 952 - Psicopedagogia

**Lo stadio dell’intelligenza sensomotoria si colloca fra**

- [ ] A. 0 - 14 mesi
- [x] B. 15 - 18 mesi
- [ ] C. 19 - 59 mesi
- [ ] D. 6 - 11 anni

## 953 - Psicopedagogia

**Lo sviluppo cognitivo, emotivo ed affettivo è un processo**

- [ ] A. Statico
- [x] B. Dinamico
- [ ] C. Solo endogeno
- [ ] D. Solo esogeno

## 954 - Psicopedagogia

**Nel gioco simbolico gli oggetti**

- [x] A. Sono interpretati in funzione dell’immaginazione
- [ ] B. Sono utilizzati per lo scopo per il quale sono stati realizzati
- [ ] C. Sono inutili
- [ ] D. Sono dannosi

## 955 - Psicopedagogia

**Nel modello formale di comunicazione l’emittente è**

- [ ] A. Il contenuto della comunicazione
- [ ] B. Un sistema di simboli
- [x] C. Colui che avvia la comunicazione
- [ ] D. Colui che riceve la comunicazione

## 956 - Psicopedagogia

**Nel processo di insegnamento/apprendimento la valutazione deve essere intesa come**

- [ ] A. Misurazione oggettiva del rendimento dell’allievo
- [ ] B. Momento di discussione e confronto tra insegnante e allievo sui risultati ottenuti
- [x] C. Elemento fondamentale per il corretto svolgimento del suddetto processo
- [ ] D. Riconoscimento dell’efficacia del lavoro dell’insegnante

## 957 - Psicopedagogia

**Nel ruolo di istruttore bisogna**

- [x] A. Valutare, indirizzare e motivare gli allievi
- [ ] B. Dare maggiore risalto agli aspetti tecnici
- [ ] C. Dare maggiore risalto agli aspetti psicologici
- [ ] D. Tenere nota dei tempi

## 958 - Psicopedagogia

**Nella relazione allievo/istruttore in che percentuale, approssimativamente, si usa il linguaggio non verbale? #####**

- [ ] A. 10% #####
- [ ] B. 50% #####
- [ ] C. 35% #####
- [x] D. 90%

## 959 - Psicopedagogia

**Nelle prime lezioni, per attirare l’attenzione nel proporre un esercizio, l’istruttore deve soprattutto**

- [ ] A. Spiegarlo bene
- [x] B. Dimostrarlo
- [ ] C. Far vedere un filmato
- [ ] D. Adottare una leadership autoritaria

## 960 - Psicopedagogia

**Parlando di comunicazione possiamo affermare che l’istruttore di nuoto**

- [x] A. Deve dare molta importanza al proprio atteggiamento di ascolto, fase fondamentale per poi non compiere leggerezze nella risposta
- [ ] B. Se troppo impegnato può liquidare velocemente l’allievo qualsiasi sia la richiesta
- [ ] C. Non deve necessariamente essere un bon comunicatore
- [ ] D. Usa come strumento di lavoro esclusivamente la voce, altre forme parallele di comunicazione sono solo secondarie

## 961 - Psicopedagogia

**Per accrescere l’autostima degli allievi può essere opportuno**

- [x] A. Ridimensionare le aspettative
- [ ] B. Mantenere le aspettative invariate
- [ ] C. Aumentare le aspettative
- [ ] D. Cambiare completamente oggetto dell’attenzione

## 962 - Psicopedagogia

**Per attivare l’attenzione nel proporre un esercizio, è importante**

- [ ] A. Spiegarlo bene
- [ ] B. Inibire gli allievi
- [ ] C. Proporlo durante la ginnastica prenatatoria
- [x] D. Dimostrarlo correttamente

## 963 - Psicopedagogia

**Per creare legami affettivi positivi occorre**

- [ ] A. Non dare troppa confidenza agli allievi
- [ ] B. Prendere informazioni sulla vita privata degli allievi
- [ ] C. Giocare spesso coi propri allievi
- [x] D. Conoscere gli allievi e trattarli singolarmente secondo le loro caratteristiche personali

## 964 - Psicopedagogia

**Per frustrazione s’intende: Un forte desiderio di soffrire**

- [ ] A. Una caratteristica tipica dell’adolescenza
- [ ] B. Una reazione aggressiva verso gli avversari
- [x] C. La percezione dell’impedimento a soddisfare le proprie aspirazioni
- [ ] D. Nessuna delle tre.

## 965 - Psicopedagogia

**Per gratificare gli allievi occorre**

- [ ] A. Utilizzare continuamente premi e punizioni
- [x] B. Lavorare per una relazione interpersonale soddisfacente
- [ ] C. Dosare premi e punizioni
- [ ] D. Istituire piccole gare interne

## 966 - Psicopedagogia

**Per insegnare occorre**

- [ ] A. Disporre delle conoscenze necessarie
- [x] B. Creare un legame affettivo, disporre un piano di apprendimento progressivo e realizzarlo
- [ ] C. Disporre dei mezzi adatti
- [ ] D. Intimidire l’allievo per conquistarne l’assoluta attenzione

## 967 - Psicopedagogia

**Per motivazione s’intende**

- [ ] A. L’esigenza di immaginare, rappresentare, sognare ad occhi aperti
- [ ] B. I motivi che spingono un allievo ad iscriversi in piscina
- [ ] C. L’esigenza di emergere, dominare, prevalere sull’altro inteso come estraneo contrapposto a se
- [x] D. La causa di un comportamento, ciò che può determinare il manifestarsi, la forza, la direzione e la persistenza di un comportamento

## 968 - Psicopedagogia

**Per sviluppo cognitivo s’intende**

- [ ] A. La maturazione della personalità
- [ ] B. La crescita del sistema nervoso
- [ ] C. Lo sviluppo degli organi e apparati
- [x] D. Lo sviluppo delle attività intellettive

## 969 - Psicopedagogia

**Per un fanciullo dai 6 agli 11 anni il gioco è prevalentemente**

- [ ] A. Simbolico (lo scopo è quello di applicare i propri schemi motori ad oggetti nuovi o situazioni immaginarie) e consente la drammatizzazione di situazioni emotivamente rilevanti.
- [ ] B. Di movimento (lo scopo è quello di ripetere uno schema motorio appreso in precedenza per appropriarsene definitivamente) o imitativo
- [ ] C. Libero
- [x] D. Caratterizzato dalle regole: capacità di cooperare, socializzare; aumentano i giochi di costruzione (realizzazione di capanne, oggetti)

## 970 - Psicopedagogia

**Professionalità dell’istruttore di nuoto è sinonimo di**

- [ ] A. Massima competenza tecnica
- [x] B. Conoscenza tecnico didattica – Capacità comunicativa e relazionale
- [ ] C. Massima conoscenza della didattica e grande capacità comunicativa
- [ ] D. Carisma e autorevolezza

## 971 - Psicopedagogia

**Qual è la distanza definita sociale, che l’istruttore adotta se deve dare disposizioni generali?**

- [ ] A. 100 metri
- [ ] B. 10 metri
- [ ] C. 40 centimetri
- [x] D. 120 centimetri

## 972 - Psicopedagogia

**Quale delle seguenti motivazioni allo sport è primaria nei bambini?**

- [ ] A. Estetica
- [ ] B. Aggregazione sociale
- [x] C. Gioco
- [ ] D. Affettiva

## 973 - Psicopedagogia

**Quale di queste affermazioni relative alla comunicazione è corretta?**

- [ ] A. La comunicazione verbale è la più efficace nell’ambiente piscina e da essa dipende la comprensione consapevole della richiesta esecutiva da parte dell’allievo
- [ ] B. La comunicazione è inutile
- [ ] C. La comunicazione verbale è caratterizzata da: linguaggio del corpo, prossemica e paralinguaggio.
- [x] D. Nel medesimo contesto, una corretta comunicazione verbale può essere invalidata da una comunicazione non verbale inefficiente

## 974 - Psicopedagogia

**Quale forma di intelligenza si sviluppa, tipicamente, durante i primi 18 mesi di vita?**

- [x] A. Intelligenza senso motoria
- [ ] B. Intelligenza preconcettuale
- [ ] C. Intelligenza operatoria
- [ ] D. Intelligenza simbolica

## 975 - Psicopedagogia

**Quale tra le seguenti affermazioni, inerenti la psicologia dei gruppi, è vera?**

- [ ] A. La leadership è una persona all’ interno del gruppo
- [ ] B. La leadership è una caratteristica specifica del leader
- [ ] C. Il manifestarsi di una leadership è deleterio per l’equilibrio endogeno del gruppo
- [x] D. La leadership è esercitata dall’ istruttore per condurre il gruppo verso un fine comune

## 976 - Psicopedagogia

**Quale tra le seguenti affermazioni, inerenti la psicologia dei gruppi, è vera?**

- [ ] A. La leadership è una persona all’interno del gruppo
- [ ] B. La leadership è una caratteristica specifica del leader
- [x] C. La leadership è utilizzata dall’istruttore per condurre il gruppo verso un fine comune
- [ ] D. L’istruttore deve esercitare una leadership democratica a prescindere dalla situazione

## 977 - Psicopedagogia

**Quale tra le seguenti aree non contribuisce in maniera determinante nella formazione della personalità dell’individuo?**

- [ ] A. Area affettivo/emotiva
- [ ] B. Area perimetrale
- [x] C. Area antropometrica
- [ ] D. Area sociale

## 978 - Psicopedagogia

**Quali elementi della personalità dell’individuo rientrano nell’area morfologico-funzionale?**

- [ ] A. L’insieme dei rapporti che si creano tra l’individuo e le persone che vivono con lui
- [ ] B. La relazione con gli altri attraverso il corpo
- [ ] C. Le capacità coordinative
- [x] D. Le caratteristiche della struttura e del funzionamento dell’organismo

## 979 - Psicopedagogia

**Quali possono essere i disturbi di una comunicazione?**

- [ ] A. Condizione, costituzione, coordinazione, controllo
- [x] B. Rumore, perdita d’informazione, fraintendimento
- [ ] C. Rumore, luce, umidità
- [ ] D. Tanti quanti se ne possono immaginare

## 980 - Psicopedagogia

**Quali possono essere le reazioni del ricevente una comunicazione?**

- [x] A. Conferma, negazione, disconferma, squalifica
- [ ] B. Negazione, rabbia, contrattazione, depressione, accettazione
- [ ] C. Presa, trazione, spinta, recupero
- [ ] D. Introduzione, apprendimento, consolidamento, ricreazione

## 981 - Psicopedagogia

**Quali sono gli aspetti che influiscono nello sviluppo della personalità?**

- [ ] A. Biologici e psicologici
- [ ] B. Biologici, meccanici, cognitivi, sociali, affettivi, metabolici
- [ ] C. Biologici e innati
- [x] D. Biologici, psicologici, cognitivi, socio- affettivi

## 982 - Psicopedagogia

**Quali sono gli elementi della comunicazione?**

- [x] A. Emittente, messaggio, destinatario, codice, canale, contesto
- [ ] B. Grammatica, semantica e sintattica
- [ ] C. Creatività, sensibilità e intraprendenza
- [ ] D. Emittente e ricevente

## 983 - Psicopedagogia

**Quali sono i contenuti della comunicazione non verbale?**

- [x] A. Conferma, disconferma e negazione
- [ ] B. Conferma e negazione
- [ ] C. Affermazione, negazione, rigetto
- [ ] D. Presa, trazione, spinta, recupero

## 984 - Psicopedagogia

**Quali sono le barriere della comunicazione a livello di canale?**

- [ ] A. Il destinatario non conosce il codice o attribuisce al segno un significato diverso
- [ ] B. Difficoltà linguistiche
- [x] C. Eccessiva distanza tra emittente e destinatario, rumore tra emittente e destinatario
- [ ] D. Significati diversi rispetto a quelli conosciuti abitualmente

## 985 - Psicopedagogia

**Quali sono le barriere della comunicazione a livello di codice?**

- [x] A. Il destinatario non conosce il codice o attribuisce al segno un significato diverso
- [ ] B. Eccessiva distanza tra emittente e destinatario, rumore tra emittente e destinatario
- [ ] C. Rumore e caos
- [ ] D. Significati diversi rispetto a quelli conosciuti abitualmente

## 986 - Psicopedagogia

**Quali tra queste modalità comunicative appartiene al linguaggio del corpo?**

- [ ] A. Contatto visivo
- [x] B. Gestualità
- [ ] C. Far divertire gli allievi
- [ ] D. Mobilità articolare

## 987 - Psicopedagogia

**Quali tra queste sono motivazioni primarie?**

- [ ] A.  Studio e ricerca
- [x] B. Gioco e agonismo
- [ ] C. Attenzione e concentrazione
- [ ] D. Corpo e psiche

## 988 - Psicopedagogia

**Quando si comunica si trasmettono messaggi**

- [x] A. Di tipo verbale, transferale e psicologico
- [ ] B. Esclusivamente di tipo verbale
- [ ] C. Di tipo verbale e psicologico
- [ ] D. Di tipo tecnico

## 989 - Psicopedagogia

**Regola fondamentale dell’istruttore-educatore è**

- [x] A. Educare all’acqua attraverso l’acqua, soddisfando i bisogni reali delle persone in considerazione delle motivazioni, del benessere e del divertimento
- [ ] B. Instaurare la tecnica degli stili nella maniera più accurata possibile
- [ ] C. Ottenere performance elevate
- [ ] D. Educare i gruppi di scuola nuoto nella maniera più autoritaria possibile al fine di favorire la gestione delle persone in maniera uniforme ed oggettiva

## 990 - Psicopedagogia

**Saper comunicare significa anche saper**

- [ ] A. Comandare
- [ ] B. Obbedire
- [ ] C. Prevedere
- [x] D. Ascoltare

## 991 - Psicopedagogia

**Secondo una corretta definizione il gruppo è**

- [x] A. Un insieme dinamico di individui che si percepiscono vicendevolmente, più o meno indipendenti per qualche aspetto
- [ ] B. Una squadra vincente
- [ ] C. Un insieme di individui
- [ ] D. Un insieme di persone legate da reciproca simpatia

## 992 - Psicopedagogia

**Soddisfare l’altro o aver piacere dal lavoro che si svolge**

- [ ] A. Sono condizioni per imparare bene
- [x] B. Sono motivazioni necessarie all’apprendimento
- [ ] C. Sono motivazioni che accelerano i tempi di apprendimento
- [ ] D. Sono attitudini personali

## 993 - Psicopedagogia

**Tendenze tipiche del pensiero infantile**

- [x] A. Egocentrismo, realismo, animismo, finalismo
- [ ] B. Ecumenismo, realismo, irredentismo
- [ ] C. Olismo, ecumenismo, realismo, animismo
- [ ] D. Olismo, realismo, misticismo, onanismo

## 994 - Psicopedagogia

**Tra le seguenti, quale area caratterizza in modo marcato la personalità di un individuo?**

- [ ] A. Area razionale
- [ ] B. Area spontanea
- [ ] C. Area istintiva
- [x] D. Area affettivo/emotiva

## 995 - Psicopedagogia

**Tra le seguenti, quale è una forma fondamentale di apprendimento?**

- [ ] A. Leggere
- [ ] B. Per induzione
- [x] C. Per prove ed errori
- [ ] D. Correre

## 996 - Psicopedagogia

**Tra le seguenti, quale è una forma fondamentale di apprendimento?**

- [ ] A. Ascoltare
- [ ] B. Ripetere
- [x] C. Apprendimento per intuizione
- [ ] D. Memorizzare

## 997 - Psicopedagogia

**Tra le seguenti, quale è una forma fondamentale di apprendimento?**

- [x] A. Per imitazione
- [ ] B. Rotolare
- [ ] C. Per coercizione
- [ ] D. Ricordare

## 998 - Psicopedagogia

**Un corretto metodo educativo - didattico dovrebbe essere basato**

- [ ] A. Sulle rispondenza alle elevate pretese di genitori e insegnanti
- [ ] B. Sulla totale autonomia lasciata all’allievo
- [x] C. Sulla possibilità di sperimentare esperienze di successo e sulla presenza di rinforzi positivi
- [ ] D. Sui ripetuti interventi volti ad evidenziare errori ed imprecisioni

## 999 - Psicopedagogia

**Un opportuno clima psicologico implica**

- [x] A. Un’atmosfera serena e ludica che sollecita la partecipazione
- [ ] B. Un complesso di tecniche per il condizionamento psicologico
- [ ] C. Una disponibilità aggressiva verso le gare
- [ ] D. Una leadership autoritaria

## 1000 - Psicopedagogia

**Una barriera della comunicazione è:**

- [x] A. Un muro che impedisce la comunicazione
- [ ] B. Un malfunzionamento degli analizzatori
- [ ] C. Un atteggiamento di un individuo che si oppone alla comunicazione
- [ ] D. Un ostacolo di qualsiasi genere che interrompe o riduce il flusso dell’informazione



Manca la 1000